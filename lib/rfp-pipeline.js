  ...
  try {
    ...
    try {
      ...
      if (embParts.length > 0) {
        ...
        // owner's projects only. Admin scans see everything (no owner
        // constraint on their pool); member scans see only their own
        // projects so their repository never bleeds into another tenant.
        // user_workspace_projects narrows within that tenant if set.
        let allProjects;
        ...
        if (userId) {
          ...
          if (wsRows.length > 0) {
            ...
            allProjects = db.prepare(ownerSql).all(...ownerParams).filter(p => wsIds.has(p.id));
            console.log(`[scan ${scanId}] workspace filter: ${wsIds.size} selected, ${allProjects.length} with embeddings (tenant-scoped)`);
          }
        }
        if (!workspaceFilter) {
          allProjects = db.prepare(ownerSql).all(...ownerParams);
        }
        const rfpTaxonomy = {
        ...
        setTimeout(() => {
          ...
          try {
            ...
            if (!now || now.status !== 'fast_ready') {
              console.log(`[scan ${scanId}] retry ${nextAttempt} skipped (status=${now?.status ?? 'deleted'})`);
              return;
            }
          } catch { return; }
          console.log(`[scan ${scanId}] retry ${nextAttempt}/${MAX_DEEP_RETRIES} firing`);
