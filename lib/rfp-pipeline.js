...
const { AI_ANALYSIS_TIMEOUT_MS, PARSE_TIMEOUT_MS, EMBED_TIMEOUT_MS } = require('./timeouts');
...
async function runRfpScanPipeline(scanId, rfpFilePath, userId = null) {
  ...
  try {
    ...
    try {
      ...
      narrativeAdvice = await withTimeout(getNarrativeAdvice(rfpData, topMatch), 60000, 'narrativeAdvice');
    } catch (e) {
