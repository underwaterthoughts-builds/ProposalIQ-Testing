...
async function runRfpScanPipeline(scanId, rfpFilePath) {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // ─── DEEP PASS BEGINS ─── everything below runs in background
    //
    // GLOBAL WATCHDOG: the deep pass is wrapped in a 5-minute timeout.
    // If any AI call hangs past its individual timeout, the watchdog
    // catches it and marks the scan 'complete' with whatever data the
    // fast pass already saved. The user gets a verdict + matches
    // immediately; deep enrichment is best-effort.
    // ═══════════════════════════════════════════════════════════════════════
    const DEEP_PASS_TIMEOUT_MS = 5 * 60 * 1000; // 5 minutes
    const deepPassStart = Date.now();

    // Helper: check if we've exceeded the deep pass budget. Called between
    // expensive steps so we can bail early and save what we have.
    function deepPassExpired() {
      return (Date.now() - deepPassStart) > DEEP_PASS_TIMEOUT_MS;
    }
    ...
    // ── Deep pass watchdog check (after winning language + structure) ────
    if (deepPassExpired()) {
      console.warn(`[scan ${scanId}] deep pass watchdog: expired after steps 5-5b (${Math.round((Date.now()-deepPassStart)/1000)}s). Saving what we have.`);
      db.prepare("UPDATE rfp_scans SET status='complete' WHERE id=? AND status='fast_ready'").run(scanId);
      return;
    }
    ...
    try {
      gaps = await withTimeout(analyseGaps(rfpDataForGaps, enrichedMatches, membersWithCV, orgProfile), 90000, 'gaps');
    } catch (e) { warn('gaps', e.message); }

    // Recompute bid score now that we have real gaps (fast pass used [])
    try {
      const sectorProjects = db.prepare("SELECT outcome FROM projects WHERE sector = ? AND outcome IN ('won','lost')").all(rfpData.sector || '');
      const sectorWon = sectorProjects.filter(p => p.outcome === 'won').length;
      const sectorWinRate = sectorProjects.length > 0 ? Math.round((sectorWon / sectorProjects.length) * 100) : 0;
    ...
    try {
      winStrategy = await withTimeout(generateWinStrategy(rfpData, enrichedMatches, gaps, orgProfile), 120000, 'winStrategy');
    ...
    let news = [];
    try {
      news = await withTimeout(getIndustryNews(rfpData, rfpText), 60000, 'news');
    } catch (e) { warn('news', e.message); }

    // ── Deep pass watchdog check (midpoint: after gaps + strategy + news) ──
    if (deepPassExpired()) {
      console.warn(`[scan ${scanId}] deep pass watchdog: expired after steps 6-10 (${Math.round((Date.now()-deepPassStart)/1000)}s). Saving what we have.`);
      // We have gaps + strategy + news at this point. Save a partial
      // deep-pass result rather than losing everything.
      try {
        db.prepare(`UPDATE rfp_scans SET gaps=?, win_strategy=?, news=?, status='complete' WHERE id=?`).run(
          JSON.stringify(gaps), JSON.stringify(winStrategy), JSON.stringify(news), scanId
        );
      } catch (e2) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 14b — SANITY CHECK ON TOP MATCHES
    ...
    try {
      ...
      if (Array.isArray(verdicts) && verdicts.length > 0) {
        ...
        let demotedCount = 0, flaggedCount = 0;
        matchData = matchData.map(m => {
          ...
          if (v.verdict === 'demote') {
            ...
            return {
              ...m,
              taxonomy_tier: 5,
              taxonomy_match: 'cross',
              sanity_demoted: true,
              sanity_warning: v.reason || 'AI sanity check flagged this as off-scope for the RFP',
            };
          }
          if (v.verdict === 'flag') {
            flaggedCount++;
            return { ...m, sanity_warning: v.reason || null };
          }
          return m;
        });
        // Re-sort after potential demotions so the cross-sector items go
        // to the bottom where they belong.
        matchData.sort((a, b) => {
          if ((a.taxonomy_tier || 5) !== (b.taxonomy_tier || 5)) {
            return (a.taxonomy_tier || 5) - (b.taxonomy_tier || 5);
          }
          return (b.match_score || 0) - (a.match_score || 0);
        });
        if (demotedCount + flaggedCount > 0) {
          console.log(`[scan ${scanId}] sanity check: demoted=${demotedCount}, flagged=${flaggedCount}`);
        }
      }
