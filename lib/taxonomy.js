...
// Infer SECTORS within a known industry by checking canonical sector names
// against the proposal text. Returns up to 2 sectors that have evidence
// in the corpus. Used together with inferIndustryFromText so untagged
// proposals get full taxonomy coverage, not just industry.
function inferSectorsFromText(text, industryName, type) {
  if (!text || !industryName) return [];
  const tax = type === 'client' ? CLIENT_TAXONOMY : SERVICE_TAXONOMY;
  const sectors = tax[industryName] || [];
  ...
  for (const sector of sectors) {
    ...
    // Full sector name appears in text — strongest signal
    if (lower.includes(secLower)) { scored.push({ sector, score: 5 }); continue; }

    // Distinctive multi-word phrases within the sector — e.g. "ceo", "leadership"
    // for "CEO / Leadership Video"
    const words = secLower.split(/[^a-z0-9]+/).filter(Boolean);
    let hits = 0;
    for (const w of words) {
      // Skip generic glue words that would match too widely
      if (['and', 'the', 'for', 'with', 'of', 'in', 'on'].includes(w)) continue;
      if (w.length < 3) continue;
      if (lower.includes(' ' + w + ' ') || lower.includes(' ' + w + 's ') || lower.includes(' ' + w + ',')) {
        hits++;
      }
    }
    // Need at least 2 distinctive word hits, OR 1 hit if the word is long+rare
    if (hits >= 2) score = 3 + hits;
    else if (hits === 1 && words.some(w => w.length >= 8)) score = 2;
...
// Returns { client_industry, service_industry, client_sectors, service_sectors }.
// Used as fallback when explicit taxonomy columns are NULL (proposals
// uploaded before the taxonomy was added).
function inferTaxonomyFromProposal(proposal) {
  ...
  const clientIndustry = inferIndustryFromText(corpus, 'client');
  const serviceIndustry = inferIndustryFromText(corpus, 'service');

  return {
    client_industry: clientIndustry,
    service_industry: serviceIndustry,
    client_sectors: inferSectorsFromText(corpus, clientIndustry, 'client'),
    service_sectors: inferSectorsFromText(corpus, serviceIndustry, 'service'),
  };
}
