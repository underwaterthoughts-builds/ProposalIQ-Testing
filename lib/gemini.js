...
const PRICING = {
  ...
  // Anthropic Claude (Wave 6 — cross-model adversarial role)
...
Return ONLY valid JSON.`;
  ...
  // Wave 6 / Tier 1 #1: cross-model proposer/critic for section drafts.
  // When ANTHROPIC_API_KEY is set, Claude proposes the draft (best at
  // long-form prose with non-invention bias), then OpenAI runs as a
  // structural critic. The critic doesn't rewrite — it returns a list
  // of fixes, which are then applied in a single OpenAI rewrite pass.
  // Falls back to the original OpenAI-then-Gemini path if Claude isn't
  // configured, so existing deployments keep working unchanged.
  let claude = null;
  try { claude = require('./claude'); } catch {}
  if (claude?.hasClaude && claude.hasClaude()) {
    try {
      const claudeRaw = await claude.claudeGenerateJson(systemSuffix, userPrompt, 5000, 'generateSectionDraft.propose');
      const proposed = safeJSON(claudeRaw);
      if (proposed?.draft && hasOpenAI()) {
        // OpenAI critic — narrow scope: return changes only, not a rewrite.
        const criticSystem = `You are a strict editor reviewing a single proposal section draft for compliance with its contract and non-invention rules. You do not rewrite. You list issues with line citations and concrete fixes.`;
        const criticUser = `Section: "${sectionName}"

DRAFT TO REVIEW:
${proposed.draft}

CONTRACT (must be honoured):
${JSON.stringify(opts?.contract || {}, null, 2)}

CHECKS (run each, list any failures):
  1. Does the draft contain a specific number, name, or percentage NOT from the RFP and NOT wrapped in [EVIDENCE NEEDED]? Quote the offending phrase.
  2. Are all [#N] citations actually present in the matched proposals supplied? List any orphans.
  3. Is target_words depth honoured (within ±20%)?
  4. Are required_content items all addressed? List any missing.
  5. Are forbidden phrases present? List them verbatim.
  6. Does the draft hallucinate a credential, certification, or past project not in the supplied evidence pool? List it.

Return ONLY JSON:
{ "issues": [ { "type": "fabrication|missing_required|forbidden|citation|depth", "quote": "exact phrase from draft", "fix": "specific change to apply" } ], "verdict": "ship_as_is | minor_edits | needs_rewrite" }`;
        let critique = null;
        try {
          const cr = await openaiGenerate(criticSystem, criticUser, 1500, 'generateSectionDraft.critique');
          critique = safeJSON(cr);
        } catch (e) { console.error('section critique failed (continuing with Claude draft):', e.message); }

        // If critic finds nothing or only ship_as_is, return Claude's draft.
        const issues = Array.isArray(critique?.issues) ? critique.issues : [];
        if (!issues.length || critique?.verdict === 'ship_as_is') {
          return { ...proposed, _ai: { drafter: claude.CLAUDE_MODEL, critique: 'clean' } };
        }

        // If issues — apply via Claude, with the critic's fix list as the
        // editing brief. Keeps Claude's voice, surgically resolves issues.
        const rewritePrompt = `Below is your previous draft and a list of issues to fix. Apply each fix conservatively — preserve voice and structure, only correct what's flagged. Do NOT add new claims; if a fix would require new evidence, replace with [EVIDENCE NEEDED: <what>].

PREVIOUS DRAFT:
${proposed.draft}

ISSUES TO FIX:
${issues.map((i, n) => `${n + 1}. [${i.type}] "${i.quote}" → ${i.fix}`).join('\n')}

Return ONLY the corrected JSON in the same schema as before:
${schema}`;
        try {
          const fixedRaw = await claude.claudeGenerateJson(systemSuffix, rewritePrompt, 5000, 'generateSectionDraft.rewrite');
          const fixed = safeJSON(fixedRaw);
          if (fixed?.draft) return { ...fixed, _ai: { drafter: claude.CLAUDE_MODEL, critique: 'applied', issue_count: issues.length } };
        } catch (e) { console.error('section rewrite failed (returning Claude original):', e.message); }
        return { ...proposed, _ai: { drafter: claude.CLAUDE_MODEL, critique: 'unfixed', issue_count: issues.length } };
      }
      if (proposed?.draft) return { ...proposed, _ai: { drafter: claude.CLAUDE_MODEL, critique: 'skipped' } };
    } catch (e) { console.error('Claude section draft failed, falling back to OpenAI:', e.message); }
  }
...
If any field fails the critique, rewrite. Then return final JSON after
  ...
  // Wave 6 / Tier 1 #3: Claude second-opinion adversarial check on the
  // verdict. Verdicts are the single highest-stakes line in the product;
  // a single LLM is wrong often enough on judgement calls that a
  // cross-model second opinion is cheap insurance. Claude reads the
  // brief + the underlying evidence and asks: "is this verdict
  // defensible?" If it disagrees, we attach a second_opinion block to
  // the brief so the UI can flag it. The user keeps decision authority;
  // we just don't pretend a single LLM is infallible on a six-figure call.
  if (primaryBrief?.verdict) {
    try {
      const claude = require('./claude');
      if (claude.hasClaude()) {
        const reviewSystem = `You are a senior bid director giving a brutally honest second opinion on another consultant's bid/no-bid verdict. You have the same evidence they had. You agree or disagree, and you say why. You don't write a brief — you write one short paragraph.`;
        const reviewUser = `THE PRIMARY VERDICT:
Decision: ${primaryBrief.verdict.decision}
Confidence: ${primaryBrief.verdict.confidence || '—'}
Top reason: ${primaryBrief.verdict.headline || primaryBrief.verdict.summary || ''}

THE UNDERLYING EVIDENCE:
${userPrompt.slice(0, 10000)}

Your task:
1. Do you agree with the verdict? (agree | disagree | partial)
2. If disagree or partial: what would have to be true for the verdict to be correct? What would change your mind?
3. One specific risk the primary brief is underweighting (or null if you agree).

Return ONLY JSON:
{
  "agreement": "agree | disagree | partial",
  "rationale": "one short paragraph — defensible second opinion in plain language",
  "underweighted_risk": "specific risk or null"
}`;
        const raw = await claude.claudeGenerateJson(reviewSystem, reviewUser, 800, 'generateExecutiveBidBrief.secondOpinion');
        const so = safeJSON(raw);
        if (so?.agreement) {
          primaryBrief.second_opinion = {
      ...
      console.error('Claude second-opinion check failed (continuing without):', e.message);
    }
  }

  if (primaryBrief) return primaryBrief;
...
async function generateFullProposal({
  ...
  for (const section of PROPOSAL_SECTIONS) {
    try {
      ...
      const result = await generateSectionDraft(
        ...
        executiveBrief,
        orgProfile,
        { gaps, suggestedApproach, narrativeAdvice, writingInsights, team, contract: section.contract }
      );
  ...
  // Wave 6 / Tier 1 #2: Claude consistency critic over the assembled
  // proposal. Per-section QA can't see across sections — it can't catch
  // "Approach says 12 weeks but Commercial says 16 weeks" or
  // "Team section names Alice but QA section names a different lead".
  // One Claude pass over the whole composed document catches that class
  // of bug. Returns a list of inconsistencies attached to the result;
  // the caller surfaces them in the UI rather than silently rewriting,
  // because cross-section fixes typically need human judgement.
  let consistencyFlags = [];
  try {
    const claude = require('./claude');
    if (claude.hasClaude()) {
      const criticSystem = `You are reviewing an assembled proposal for cross-section consistency. You do not rewrite. You list specific inconsistencies between sections that a human reviewer would want to resolve before submission.`;
      const criticUser = `ASSEMBLED PROPOSAL (${completedSections.length} sections, ~${fullProposal.length.toLocaleString()} chars):

${fullProposal}

CHECKS — for each, list any issues with the offending quote AND the section names where the conflict occurs:
  1. Timeline conflicts — does any number-of-weeks / months / phase-duration claim disagree across sections?
  2. Budget conflicts — do figures (low/mid/high, per-workstream) disagree across sections?
  3. Team conflicts — does a person named in one section have a different role / credential / responsibility in another?
  4. Scope conflicts — does a deliverable described one way in Approach contradict its description in Commercial or Assembly?
  5. Currency / locale conflicts — currency used inconsistently?
  6. Citation conflicts — does a [#N] or named past project reference inconsistently across sections?

Return ONLY JSON:
{ "verdict": "consistent|minor_drift|conflicting", "flags": [ { "type": "timeline|budget|team|scope|currency|citation", "sections": ["Section A name", "Section B name"], "conflict": "specific phrase from each section", "recommended_fix": "what the user should reconcile before submission" } ] }`;
      const raw = await claude.claudeGenerateJson(criticSystem, criticUser, 2500, 'generateFullProposal.consistency');
      const parsed = safeJSON(raw);
      if (Array.isArray(parsed?.flags)) consistencyFlags = parsed.flags;
    }
  } catch (e) {
    console.error('Full-proposal consistency critic failed (continuing without):', e.message);
  }
