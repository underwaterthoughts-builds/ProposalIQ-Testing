...
// OpenAI call that returns plain text (no JSON mode). Used for the style
// conformance rewrite which outputs prose, not structured data.
async function openaiGenerateText(systemSuffix, userPrompt, maxTokens = 4000) {
  const client = openaiClient();
  const system = systemSuffix ? `${MASTER_SYSTEM}\n\n${systemSuffix}` : MASTER_SYSTEM;
  const res = await client.chat.completions.create({
    model: OPENAI_MODEL,
...
phrase or section in the document. If you cannot point to one, drop it.`;
  ...
  "service_industry": "pick EXACTLY ONE from this list — what TYPE OF WORK was delivered. Use the exact string. If none clearly fit, return null: ${SERVICE_INDUSTRY_ENUM}",
  "service_sectors": ["UP TO 3 sub-sectors from the chosen service industry — must exactly match canonical sector names"],
  "client_industry": "pick EXACTLY ONE from this list — what SECTOR the client operates in. Use the exact string. If none clearly fit, return null: ${CLIENT_INDUSTRY_ENUM}",
...
❌ DO NOT INVENT requirements that have no basis in the document or
  ...
  "service_industry": "pick EXACTLY ONE — what TYPE OF WORK is this RFP asking for. Use the exact string. If unclear, return null: ${SERVICE_INDUSTRY_ENUM}",
  "service_sectors": ["UP TO 3 sub-sectors from the chosen service industry that match what this RFP needs"],
  "client_industry": "pick EXACTLY ONE — what SECTOR is the issuing organisation in. Use the exact string. If unclear, return null: ${CLIENT_INDUSTRY_ENUM}",
...
══════════════════════════════════════════════════════════════════════════
  ...
  □ For service_industry/client_industry, does the closed-list value
    you picked match the document, or did you guess?
...
══════════════════════════════════════════════════════════════════════════
  a. List every MUST requirement. For each, is it directly addressed
     by at least one matched proposal? If yes, not a gap.
     If the ORGANISATION CONTEXT above shows we DON'T have this
     capability at all, that's a genuine capability gap (not fakeable
     within the bid).
  b. List every implicit requirement. For each, do the matched
     proposals demonstrate the relevant evidence? Weak evidence → gap.
  c. Of the gaps you've identified, group any that overlap and pick
     the strongest framing for each group.
  d. Rank by impact on win probability. Cut anything below the bar.
  e. For each survivor, identify whether a specific team member is
     genuinely a fit (check their actual specialisms/CV — do not
     suggest people who only loosely match).

══════════════════════════════════════════════════════════════════════════
STEP 2 — Draft 5–8 gaps using this schema:
══════════════════════════════════════════════════════════════════════════
${schema}

══════════════════════════════════════════════════════════════════════════
STEP 3 — Self-critique. Before returning, review:
══════════════════════════════════════════════════════════════════════════
...
JSON after </thinking>. Return ONLY valid JSON.`;
  ...
  try {
    if (hasOpenAI()) {
      const raw = await openaiGenerate(systemSuffix, userPrompt, 4000);
      const parsed = safeJSON(raw);
...
async function generateSectionDraft(sectionName, sectionDescription, rfpData, matchedProposals, winStrategy, winningLanguage, executiveBrief = null, orgProfile = null) {
  ...
  const systemSuffix = `You are a senior proposal writer generating first-draft content for the "${sectionName}" section of a real bid.

═══════════════════════════════════════════════════════════════════════════
WHAT THIS DRAFT IS
═══════════════════════════════════════════════════════════════════════════
A first-pass that the bid writer will edit, not finished prose. Your job is
to give them a strong starting point grounded in the matched past work,
NOT to fabricate details to make it look polished.

═══════════════════════════════════════════════════════════════════════════
HARD RULES
═══════════════════════════════════════════════════════════════════════════
  ✓ Write in first person plural (we/our)
  ✓ Reference matched proposals BY INDEX TAG ([#1], [#2]) wherever you
    cite their experience — the UI converts these into clickable links
  ✓ Use winning language snippets as INSPIRATION ([L1], [L2]) — adapt
    the rhetorical move, never copy the exact words
  ✓ Use [EVIDENCE NEEDED: specific thing] markers wherever the writer
    must insert real data (e.g. "[EVIDENCE NEEDED: HMRC contract value]")
  ✓ NEVER invent client names, project names, dates, numbers, percentages,
    or quotes that aren't in the source data
  ✓ Aim for 150-250 words unless the section type needs more
  ✓ Write prose, not bullet points, unless the section is naturally a list

═══════════════════════════════════════════════════════════════════════════
EXAMPLES — what to do vs not to do
═══════════════════════════════════════════════════════════════════════════

❌ INVENTED FACT (do not do):
   "We delivered the Met Office's £12M migration with zero downtime over
    18 months across 47 trusts."
   (Even if these numbers came from a matched proposal, do not lift them
    verbatim into a different bid context.)

✅ SOURCE-CITED with placeholder:
   "We have shipped this exact integration pattern in production at scale —
    most recently for [#1], where we delivered [EVIDENCE NEEDED: the
    specific outcome from this proposal] without disrupting live services."

❌ GENERIC FILLER:
   "Our team brings deep expertise and a proven track record."

✅ STRUCTURALLY LIFTED from winning language [L2]:
   "Three things distinguish this proposal from a competent alternative,
    and we will prove each one in the section that follows: [EVIDENCE
    NEEDED: differentiator 1], [EVIDENCE NEEDED: differentiator 2], and
    [EVIDENCE NEEDED: differentiator 3]."
...
${hiddenExpectations.map(h => `  · ${h}`).join('\n') || '  (none)'}`;
  ...
  const systemSuffix = `You are a senior proposal writer drafting a COMPLETE, submission-ready proposal for a real bid.

═══════════════════════════════════════════════════════════════════════════
WHAT THIS DOCUMENT IS
═══════════════════════════════════════════════════════════════════════════
A first-draft proposal the bid team can edit, refine, and submit. Not a
summary. Not a plan. An actual proposal document that a procurement
evaluator would read and score.

Every section should be written as if the bid team's reputation depends
...
═══════════════════════════════════════════════════════════════════════════
  ...
  ✓ Write prose, not bullet points, unless the section naturally needs a list
  ✓ Be SPECIFIC to this client, sector, and RFP — no filler
  ✓ Every claim must be backed by evidence from the matched proposals or
    wrapped in [EVIDENCE NEEDED: specific thing] markers
  ✓ Reference matched proposals by [#N] tag where you use their evidence
  ✓ Use winning language snippets as STRUCTURAL INSPIRATION — adapt the
    rhetorical move, never copy the exact words
  ✓ Address every MUST requirement explicitly (evaluators check these off)
  ✓ Address the top SHOULD requirements where relevant
  ✓ Pre-empt the HIGH-PRIORITY GAPS — show you've thought about them
  ✓ The executive summary must LEAD with the winning thesis
  ✓ The approach must reference the methodology from the won proposals
  ✓ The team section must name real people with real credentials
  ✓ The close must be confident, specific, and forward-looking
  ✓ Leave [EVIDENCE NEEDED: X] markers wherever specific data is required
    that you don't have — NEVER fabricate numbers, names, or dates

═══════════════════════════════════════════════════════════════════════════
TONE AND STYLE
═══════════════════════════════════════════════════════════════════════════
${styleHint || 'Write in a confident, direct, evidence-led professional tone.'}

═══════════════════════════════════════════════════════════════════════════
STRUCTURE — write these sections in this order, with markdown ## headers
═══════════════════════════════════════════════════════════════════════════
${structureHint ? `Structural guidance from your best-matched won proposals:\n${structureHint}\n` : ''}
1. ## Executive Summary (200-300 words)
   Lead with the winning thesis. State who you are, why you're the right
   choice, and the 3 things that differentiate this response. End with
   a forward-looking statement about the partnership.

2. ## Understanding of the Requirement (200-400 words)
   Demonstrate you understand what the client ACTUALLY needs — including
   implicit expectations. Reference specific RFP requirements by theme.
   Show insight, not just paraphrasing.

3. Our Proposed Approach (400-700 words — the most important section)

   BEFORE WRITING: interrogate the brief. Ask yourself:
     · What is the client actually trying to ACHIEVE — not what they
       asked for, but what outcome would make this project a success?
     · What are the 2-3 hardest problems hidden in this brief?
     · What has gone wrong when similar projects fail?
     · What specific risks does THIS client face in THIS context?

   Then write the approach to answer those questions, not just to list
   phases. Structure it as:

   a) OUTCOME FRAMING (2-3 sentences): Start by naming the outcome
      this approach delivers — not what you'll do, but what the client
      will have at the end. Be specific: "a live, production-grade
      integration handling [X] transactions/day" not "a delivered system".

   b) WHY THIS APPROACH (2-3 sentences): Explain the strategic logic
      behind your methodology choice. Reference what worked in similar
      engagements (cite matched proposals). Name what you're deliberately
      NOT doing and why — this shows strategic thinking.

   c) PHASED DELIVERY (the bulk): 3-5 phases, each with:
      · Phase name and duration
      · What it produces (a concrete deliverable, not an activity)
      · Why this phase comes at this point (the dependency logic)
      · Which risk it mitigates or which requirement it addresses
      Ground each phase in evidence from matched proposals — "this
      mirrors the phasing that succeeded at (Proposal: 'X')".

   d) OUTCOMES & MEASURES (2-3 sentences): What success looks like in
      concrete, measurable terms. What the client can expect to see at
      3 months, 6 months, project end. Be specific — not "improved
      performance" but "processing time reduced from [X] to [Y]" or
      [EVIDENCE NEEDED: target metric].

   e) PRE-EMPT GAPS (1-2 sentences): Directly address the top
      high-priority gap from the gap analysis. Show you've anticipated
      the evaluator's concern before they raise it.

   This section should make the evaluator think "they've actually thought
   about how to deliver this, not just listed what they'll do."

4. ## Relevant Experience & Case Studies (300-500 words)
   2-3 specific case studies drawn from the matched proposals. For each:
   client context, what you delivered, measurable outcome. Tie each
   case study to a specific RFP requirement it demonstrates.

5. ## Our Team (200-300 words)
   Key personnel with named roles, relevant credentials, and why each
   person is the right fit for this engagement. Reference their track
   record on the cited case studies.

6. ## Quality Assurance & Risk Management (150-250 words)
   Named methodology, specific risks for this engagement (drawn from
   the gap analysis), and concrete mitigations.

7. ## Commercial Proposal (100-200 words)
   Framework for pricing (not specific numbers unless available).
   Reference the approach phases. State assumptions clearly.

8. ## Why [Client Name] Should Choose Us (150-200 words)
   Closing argument. Reinforce the winning thesis. Reference the
   strongest proof point. End with a clear, confident call to action.

═══════════════════════════════════════════════════════════════════════════
DO NOT
═══════════════════════════════════════════════════════════════════════════
  ❌ Use generic filler ("proven track record", "trusted partner",
     "passion for excellence", "dedicated team")
  ❌ Invent specific numbers, dates, client names, or quotes
  ❌ Write section headings like "Section 1" — use the actual titles
  ❌ Make the executive summary a table of contents
  ❌ Repeat the same evidence in multiple sections
  ❌ Skip any section — write all 8`;
