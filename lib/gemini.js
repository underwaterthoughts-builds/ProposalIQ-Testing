...
- Organisation-profile fit: if the caller supplied a confirmed profile
  (see block below), check whether the past proposal sits inside the
  CURRENT confirmed offerings. A past proposal in an offering the org
  no longer confirms (or never did) is a weaker fit even if it matches
  thematically — the skill/delivery capability may have lapsed, and the
  bid team shouldn't build on capability the org has walked away from.
  Reflect this in the relevance_score (cap at 60 when the past proposal
  is clearly outside current confirmed offerings and the RFP is clearly
  inside them) and call it out in not_relevant_for. Do NOT apply this
  adjustment when no profile is supplied.
...
top despite being irrelevant because:
  ...
  · Organisation-profile fit: if a confirmed organisation profile is
    supplied, flag matches that fall outside the CURRENT confirmed
    offerings and sectors. These are references to capability the org
    may have walked away from; surfacing them as top matches can
    mislead the bid team. Prefer "flag" with the caveat "outside
    current confirmed offerings" over "demote" unless the mismatch is
    egregious. When no profile is supplied, do NOT make this adjustment.
