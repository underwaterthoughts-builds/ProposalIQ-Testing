...
${(() => {
  ...
  if (suggestedApproach && typeof suggestedApproach === 'object') {
    ...
    if (budget && typeof budget === 'object') {
      const cur = budget.currency || 'GBP';
      const fmt = (v) => v ? `${cur === 'GBP' ? '£' : cur + ' '}${(v / 1000).toFixed(0)}K` : null;
      const parts = [fmt(budget.low) && `LOW ${fmt(budget.low)}`, fmt(budget.mid) && `MID ${fmt(budget.mid)}`, fmt(budget.high) && `HIGH ${fmt(budget.high)}`].filter(Boolean);
      if (parts.length > 0) {
        bits.push('', `INDICATIVE BUDGET (use these in the Commercial section, do not paraphrase as "tiered approach"): ${parts.join(' · ')}`);
  ...
  // ── Team records — used by the Team section especially, and by any
  // section that names a phase owner. The model MUST use real names from
  // this list when a member's title matches the role required.
  const teamRecords = Array.isArray(team) ? team.slice(0, 20) : [];
  if (teamRecords.length > 0) {
    bits.push('', 'TEAM RECORDS (real team members — use real names in the Team section and anywhere a phase owner is referenced; only [TBC: <role>] when no member matches the role):');
    ...
    bits.push('', 'TEAM RECORDS: none supplied. The Team section may use [TBC: <role>] markers — these will be highlighted to the user for input.');
  }
...
const SECTION_CONTRACTS = {
  ...
  approach: {
    ...
    forbidden: ['One-line phase descriptions', 'Phases without team roles or rationale', 'Generic phrases'],
  },
  experience: {
  ...
  commercial: {
    ...
    description: 'Pricing structure with explicit numbers from suggestedApproach budget. Key assumptions. Inclusions/exclusions. How scope changes are handled.',
    target_words: 400,
    snippet_use_cases: ['commercial', 'value proposition'],
    required_content: [
      'Indicative budget LOW / MID / HIGH from suggestedApproach.indicative_budget when supplied — actual numbers, not "tiered approach"',
    ...
    forbidden: ['Vague phrases like "tiered approach" without any number', 'Missing exclusions list'],
  },
  why_us: {
