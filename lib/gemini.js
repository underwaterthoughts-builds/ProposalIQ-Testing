...
// Per-request debug logging. Off by default — set DEBUG=1 in the env to
// see cache-hit / work-type / market-context / vision diagnostics.
...
const PRICING = {
  ...
  // ESTIMATES for the 2026-09 Gemini-primary stopgap — verify against
  // ai.google.dev/pricing and correct before trusting the AI-costs page.
  ...
  // Anthropic Claude (used by the in-app assistant)
...
// ── GEMINI SETUP ──────────────────────────────────────────────────────────────
// Deduped so each model is only tried once per request. Pro-first chain since
// the 2026-09 Gemini-primary stopgap (OpenAI billing outage): quality of the
// deep analysis steps is the product, so lead with the strongest available
// model and degrade through flash tiers. Verified available to our key via
// ListModels on 2026-09-01. `flash-latest` is Google's always-valid alias,
// kept as a true last resort.
...
// Tasks that prefer Gemini Flash over OpenAI (Wave 6 Phase 3 cost
// optimisation). These are mechanical / extraction / classification
// tasks where the small quality drop from gpt-4o → flash is invisible
// in output but saves ~$0.23/scan on average. OpenAI remains the
// fallback if Gemini errors. Override per-task via env var
// PIQ_PREFER_FLASH_<NAME>=false (e.g. PIQ_PREFER_FLASH_qaDetectIssues=false).
const FLASH_PREFERRED_TASKS = new Set([
  'explainMatch',
  'extractSearchableEntities',
  'scoreAndCategoriseMarketContext',
  'reRankProposals',
]);

function preferFlashFor(name) {
  if (process.env[`PIQ_PREFER_FLASH_${name}`] === 'false') return false;
  return FLASH_PREFERRED_TASKS.has(name);
}

// Embedding models, tried in order. text-embedding-004 was RETIRED by Google
// (404s since mid-2026) — that outage produced zero-match scans; keep this
// list to models verified via ListModels. All embeddings MUST come from the
// same model family at the same dimensionality: cosine() returns 0 for
// mixed-dimension vectors, and even same-length vectors from different
// models live in incompatible spaces. Changing this list requires re-running
// scripts/reembed.js against every stored embedding.
const EMB_MODELS = [
  'gemini-embedding-2',
  'gemini-embedding-001',
];
// Matches the dimensionality of the pre-2026-09 OpenAI text-embedding-3-small
// vectors so the schema/storage shape is unchanged. cosine() normalises, so
// no client-side re-normalisation is needed for truncated MRL embeddings.
...
// PIQ_DISABLE_OPENAI=1 is the Gemini-primary kill switch: with it set, every
// call routes straight to Gemini with no doomed OpenAI attempt first, and
// analysis_model labels honestly. Unset it (and provide a valid key) to
// restore OpenAI-primary.
function hasOpenAI() {
  if (process.env.PIQ_DISABLE_OPENAI === '1') return false;
...
CLIENT REFERENCES ARE PROOF POINTS, NOT TEMPLATES:
When a past client or project is referenced in any draft, narrative, or
recommendation, the structure must be:
  1. Capability or approach stated first (in the org's own voice).
  2. Past client cited second, as evidence the capability has been
     delivered before.

❌ BAD (reads as "we will do what we did for X"):
   "Our approach draws on our success with the Philips Experience Series,
    known for its dynamic storytelling and audience connection, proving
    we can craft strategic, emotionally resonant narratives."

✅ GOOD (capability first, client as proof point):
   "Our approach draws on previous successes creating work with dynamic
    storytelling and audience connection, notably with the Philips
    Experience Series, crafting strategic, emotionally resonant
    narratives."

The bad form makes a past client engagement look like a template the org
is repeating; the good form establishes a capability that the past
client engagement validates. This rule applies anywhere a past client,
project, or matched proposal is named — drafts, narrative advice, win
strategy, executive briefs, and language adaptations alike.`;
...
async function _openaiCallWithRetry(params, _fnName) {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (usage) {
        // OpenAI gpt-4o auto-caches prompt prefixes >1024 tokens that are
        // byte-identical across calls. Our system messages always start
        // with MASTER_SYSTEM (~1500 tokens), so the cache hits whenever
        // we make a follow-up call with the same MASTER prefix. The
        // per-call savings are visible in usage.prompt_tokens_details.
        // cached_tokens; logging surfaces them so the dev can verify
        // the cache is working (and budget the right token/cost).
        const cached = usage?.prompt_tokens_details?.cached_tokens || 0;
        if (cached > 0) {
          dlog(`[ai-cost ${_fnName}] cached_tokens=${cached} of ${usage.prompt_tokens} input (~${Math.round(100 * cached / usage.prompt_tokens)}% cache hit)`);
        }
        logAiCost(params.model, usage.prompt_tokens, usage.completion_tokens, _fnName);
      }
      return res;
    } catch (e) {
      lastErr = e;
...
// gpt-5.x and o-series reasoning models reject the legacy `max_tokens`
// parameter and require `max_completion_tokens` instead. gpt-4.x and
// older still accept max_tokens. Pick the right parameter name based
// on the configured model family. Without this, switching the env var
// to gpt-5.5 caused every OpenAI call to 400 with "Unsupported
// parameter: 'max_tokens' is not supported with this model."
function tokenLimitParam(model, maxTokens) {
  if (/^(gpt-[5-9]|gpt-[1-9][0-9]|o[1-9])/.test(model || '')) {
    return { max_completion_tokens: maxTokens };
  }
  return { max_tokens: maxTokens };
}

// Hard-coded model overrides for high-stakes call sites. The five
// call sites that pass these aren't governed by OPENAI_MODEL because
// the cost/quality trade-off is decided per call:
//   - DEEP: scoring + executive brief — flagship reasoning, premium
//     cost is justified by output users actually read.
//   - REASONING: rule-application QA — o-series spots edge cases the
//     standard models miss, at a fraction of the cost of pro models.
const MODEL_DEEP = 'gpt-5.5-pro';
const MODEL_REASONING = 'o4-mini';

// Fast-mode default — used when an RFP scan is started in 'fast' mode.
// gpt-4o is fast and competent for the bulk extraction/scoring/news work
// that doesn't benefit from gpt-5.5's deeper reasoning. Tier-1 (DEEP) and
// tier-5 (REASONING) overrides above still apply in fast mode — only the
// global default flips. Override-able via OPENAI_MODEL_FAST env var.
const MODEL_FAST = process.env.OPENAI_MODEL_FAST || 'gpt-4o';

// Active per-pipeline model override. The pipeline calls
// setActiveScanModel('gpt-4o') when running in fast mode; openaiGenerate
// reads it as the default when no explicit modelOverride is passed and
// no OPENAI_MODEL is set on the call site. Cleared (set to null) when
// the pipeline finishes so subsequent work uses the env default.
//
// Concurrency caveat: this is module-level state, same as setCostContext.
// Two simultaneous scans on the same process would share it. Acceptable
// for now — the pipeline is fire-and-forget per request and concurrent
// scans are rare; if it becomes a problem we'll thread a context object
// through every call site.
let _activeScanModel = null;
function setActiveScanModel(model) { _activeScanModel = model || null; }

// o-series reasoning models reject `response_format: json_object` and
// don't accept the standard `system` role the same way; merge system
// into the user message and let the JSON requirement live in the
// prompt itself. Returns the messages array shape the model accepts.
function buildMessages(model, system, userPrompt, jsonMode = true) {
  if (/^o[1-9]/.test(model || '')) {
    const inlineJson = jsonMode ? '\n\nReturn ONLY valid JSON. No code fences, no commentary.' : '';
    return [{ role: 'user', content: `${system}\n\n${userPrompt}${inlineJson}` }];
  }
  return [
...
async function openaiGenerate(systemSuffix, userPrompt, maxTokens = 4000, _fnName = 'openaiGenerate', modelOverride = null) {
  const model = modelOverride || _activeScanModel || OPENAI_MODEL;
  const system = systemSuffix ? `${MASTER_SYSTEM}\n\n${systemSuffix}` : MASTER_SYSTEM;
  const isReasoning = /^o[1-9]/.test(model);
  const params = {
    model,
    messages: buildMessages(model, system, userPrompt, true),
...
async function openaiGenerateText(systemSuffix, userPrompt, maxTokens = 4000, _fnName = 'openaiGenerateText', modelOverride = null) {
  const model = modelOverride || _activeScanModel || OPENAI_MODEL;
  const system = systemSuffix ? `${MASTER_SYSTEM}\n\n${systemSuffix}` : MASTER_SYSTEM;
  const params = {
    model,
    messages: buildMessages(model, system, userPrompt, false),
    ...tokenLimitParam(model, maxTokens),
  };
  const res = await openaiEnqueue(() => _openaiCallWithRetry(params, _fnName));
  return res.choices[0].message.content || '';
...
// Renders the partnership-bid context (delivery partners + named CVs
// attached to this specific scan). Emitted by buildOrgProfileBlock so it
// reaches every prompt that receives org context.
function buildBidTeamBlock(orgProfile) {
  const partners = Array.isArray(orgProfile?.bid_partners) ? orgProfile.bid_partners : [];
  const cvs = Array.isArray(orgProfile?.bid_cvs) ? orgProfile.bid_cvs : [];
  if (partners.length === 0 && cvs.length === 0) return '';

  const lines = [
    '═══════════════════════════════════════════════════════════════════════════',
    'BID TEAM FOR THIS PITCH — partnership delivery (user-supplied, this bid only)',
    '═══════════════════════════════════════════════════════════════════════════',
    'This bid is being delivered in partnership. Every capability and person',
    'listed below IS AVAILABLE to this bid. Do NOT raise a capability gap,',
    'missing-personnel gap, or compliance risk for anything a partner or a',
    'named CV below demonstrably covers — instead treat it as covered and,',
    'where useful, note that it is delivered through the partnership.',
    'Reference partners by name in strategy and drafts where it strengthens',
    'the bid (joint delivery is a credibility point, not a weakness).',
  ];
  if (partners.length > 0) {
    lines.push('', 'Delivery partners on this bid:');
    partners.forEach(p => {
      lines.push(`  · ${p.name}${p.website ? ` (${p.website})` : ''}${p.capabilities ? ` — brings: ${p.capabilities}` : ''}`);
    });
  }
  if (cvs.length > 0) {
    lines.push('', `Named personnel with CVs attached (${cvs.length}):`);
    cvs.slice(0, 8).forEach(cv => {
      const excerpt = (cv.extracted_text || '').replace(/\s+/g, ' ').trim().slice(0, 700);
      lines.push(`  · ${cv.person_name || cv.original_name}${excerpt ? ` — CV extract: ${excerpt}` : ' (CV attached, no text extracted)'}`);
    });
  }
  return lines.join('\n');
}
...
function buildOrgProfileBlock(orgProfile) {
  const bidTeam = buildBidTeamBlock(orgProfile);
  if (!orgProfile || !orgProfile.confirmed_profile) return bidTeam;
  const c = orgProfile.confirmed_profile;
  const offerings = Array.isArray(c.offerings) ? c.offerings : [];
  if (offerings.length === 0) return bidTeam;
  ...
  // Bid-team block comes AFTER the confirmed-offerings constraint on
  // purpose: for THIS bid, partner capabilities and named CVs extend the
  // org's own confirmed offerings.
  return [lines.filter(l => l !== null).join('\n'), bidTeam].filter(Boolean).join('\n\n');
}
...
function buildRfpContextBlock(rfpData) {
  ...
  // RFP-stated budget range and currency — if present, the commercial
  // section MUST quote in this currency and align to (or explicitly
  // justify deviating from) this range.
  if (rfpData.budget_range && (rfpData.budget_range.low || rfpData.budget_range.high || rfpData.budget_range.raw)) {
    const cur = rfpData.currency || '';
    const lo = rfpData.budget_range.low ? `${cur} ${Number(rfpData.budget_range.low).toLocaleString()}` : '';
    const hi = rfpData.budget_range.high ? `${cur} ${Number(rfpData.budget_range.high).toLocaleString()}` : '';
    const range = (lo && hi) ? `${lo} – ${hi}` : (rfpData.budget_range.raw || '');
    bits.push(`STATED BUDGET RANGE: ${range}${rfpData.currency ? ` (${rfpData.currency})` : ''}`);
  } else if (rfpData.currency) {
    bits.push(`Currency: ${rfpData.currency}`);
  }
  if (rfpData.timeline) bits.push(`Timeline: ${rfpData.timeline}${rfpData.timeline_weeks ? ` (${rfpData.timeline_weeks} weeks total)` : ''}`);
  const deliverables = Array.isArray(rfpData.deliverables) ? rfpData.deliverables : [];
  if (deliverables.length > 0) bits.push(`MANDATORY DELIVERABLES (each MUST be addressed in the response): ${deliverables.join('; ')}`);
  ...
  const priorities = Array.isArray(rfpData.background_priorities) ? rfpData.background_priorities : [];
  if (priorities.length > 0) bits.push(`CLIENT BACKGROUND PRIORITIES (engage with these in Understanding / Approach / Why-Us): ${priorities.join('; ')}`);
...
// Domain-fit team filter. Returns only those team members whose stated
// title or specialisms plausibly match the requested service industry.
// The goal is to prevent a consulting / IT bench being stuffed into a
// film proposal (or vice versa). Loose substring match — generous on
// purpose; the section prompt still has the final say on whether to
// name a member or fall back to [TBC: <role>].
function filterTeamByDomainFit(team, serviceIndustry) {
  if (!Array.isArray(team) || team.length === 0) return [];
  if (!serviceIndustry) return [];
  const ind = String(serviceIndustry).toLowerCase();

  // Per-domain canonical role / specialism keywords. Loose by design.
  const KEYWORDS = {
    'film, video & audio': ['director', 'producer', 'cinematographer', 'dop', 'editor', 'colour', 'sound', 'animator', 'motion', 'storyboard', 'scriptwriter', 'creative', 'film', 'video', 'documentary', 'production'],
    'creative strategy & brand': ['creative director', 'brand strategist', 'planner', 'art director', 'designer', 'copywriter', 'creative', 'brand', 'strategy', 'campaign'],
    'design & experience': ['designer', 'ux', 'ui', 'experience', 'visual', 'creative director', 'art director', 'studio'],
    'advertising & campaigns': ['creative director', 'art director', 'copywriter', 'strategist', 'planner', 'producer', 'campaign', 'account director', 'creative', 'campaign'],
    'pr & media relations': ['account director', 'pr', 'comms', 'media relations', 'press', 'spokesperson'],
    'social & digital content': ['social', 'content', 'community', 'editor', 'producer', 'creative'],
    'corporate & internal communications': ['comms', 'internal communications', 'change', 'employee engagement'],
    'events & experiential': ['event', 'experiential', 'producer', 'production manager', 'live events'],
    'content & editorial': ['editor', 'writer', 'journalist', 'copy', 'editorial', 'content'],
    'engineering & technical': ['engineer', 'technical', 'architect', 'developer', 'devops'],
    'technology & digital': ['developer', 'engineer', 'architect', 'devops', 'product manager', 'technical', 'data'],
    'management consulting': ['consultant', 'manager', 'analyst', 'strategy'],
    'strategy & advisory': ['strategy', 'advisor', 'consultant', 'planner'],
    'research, insights & planning': ['researcher', 'analyst', 'planner', 'insight', 'strategist'],
    'people & organisational development': ['hr', 'l&d', 'organisational', 'change', 'consultant'],
    'programme & project management': ['project manager', 'programme manager', 'pmo', 'delivery'],
    'finance & commercial': ['accountant', 'cfo', 'commercial', 'analyst', 'pricing', 'finance'],
    'legal & regulatory': ['legal', 'counsel', 'regulatory', 'compliance', 'paralegal'],
    'sustainability & esg': ['sustainability', 'esg', 'environmental', 'carbon'],
  };

  // Domain "families" — used to drive negative-keyword exclusions when the
  // service industry doesn't have an explicit positive list, OR to catch
  // members whose titles fit the positive list loosely but are clearly
  // wrong-domain (e.g. "Lead Solutions Architect" matches "architect" in
  // some lists but is unmistakably IT, not creative).
  const CREATIVE_FAMILIES = new Set([
    'film, video & audio', 'creative strategy & brand', 'design & experience',
    'advertising & campaigns', 'pr & media relations', 'social & digital content',
    'corporate & internal communications', 'events & experiential', 'content & editorial',
  ]);
  const TECH_FAMILIES = new Set([
    'engineering & technical', 'technology & digital',
  ]);

  // Hard-block keywords that signal a member is unmistakably from a
  // different family. These override positive matches.
  const NEGATIVE_FOR_CREATIVE = [
    'solutions architect', 'lead solutions architect', 'devops', 'sre',
    'site reliability', 'data engineer', 'infrastructure', 'network engineer',
    'cybersecurity', 'cloud architect', 'platform engineer', 'integration architect',
    'sap consultant', 'oracle consultant', 'salesforce', 'kubernetes',
    'machine learning engineer', 'ml engineer', 'database administrator', 'dba',
  ];
  const NEGATIVE_FOR_TECH = [
    'creative director', 'art director', 'copywriter', 'cinematographer',
    'dop', 'colourist', 'sound designer', 'choreographer',
  ];

  const wanted = KEYWORDS[ind] || [];
  const isCreative = CREATIVE_FAMILIES.has(ind);
  const isTech = TECH_FAMILIES.has(ind);

  const excluded = isCreative ? NEGATIVE_FOR_CREATIVE
                  : isTech ? NEGATIVE_FOR_TECH
                  : [];

  const result = team.filter(m => {
    const hay = [
      m.title || '',
      ...(Array.isArray(m.stated_specialisms) ? m.stated_specialisms : [m.stated_specialisms || '']),
      m.stated_sectors || '',
      m.bio || '',
    ].join(' | ').toLowerCase();

    // Hard exclusions first — wrong-domain members are dropped even if
    // they coincidentally match a positive keyword (e.g. "Lead Solutions
    // Architect" → has "architect" but is plainly IT in a creative bid).
    if (excluded.some(neg => hay.includes(neg))) return false;

    // For domains with a positive keyword list, require at least one match.
    if (wanted.length > 0) return wanted.some(w => hay.includes(w));

    // For unmapped domains, fall back to "no obvious wrong-domain markers"
    // — at least we won't ship a creative bid with PRINCE2-toting consultants.
    return true;
  });

  dlog(`[filterTeamByDomainFit] service_industry="${serviceIndustry}" — ${result.length}/${team.length} members passed`);
  return result;
}
...
// ── EMBEDDINGS (Gemini only — deliberately NO cross-provider fallback) ────────
// A fallback to another provider's embedding model would "succeed" while
// producing vectors in an incompatible space, silently corrupting matching
// (every similarity would be near-zero or garbage). Embeddings fail LOUDLY
// instead; callers degrade the scan and tell the user matching is
// unavailable. Uses the REST endpoint directly because the legacy
// @google/generative-ai SDK does not pass outputDimensionality through.
  ...
  for (const modelName of EMB_MODELS) {
    try {
      const resp = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${modelName}:embedContent?key=${key}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            content: { parts: [{ text: textSlice }] },
            outputDimensionality: EMB_DIMENSIONS,
          }),
        }
      );
      const data = await resp.json();
      if (data?.embedding?.values?.length === EMB_DIMENSIONS) {
        return data.embedding.values;
      }
      lastErr = new Error(`${modelName}: ${JSON.stringify(data?.error || data).slice(0, 200)}`);
      // Model missing/retired → try the next one; anything else is fatal
      if (data?.error?.code !== 404) throw lastErr;
    } catch (e) {
      lastErr = e;
      if (!(e.message || '').includes('404')) throw new Error('Gemini embedding failed: ' + e.message);
    }
  }
  throw new Error('All Gemini embedding models unavailable: ' + (lastErr?.message || 'unknown'));
}

// ── WORK-TYPE DETECTION (for methodology grading) ─────────────────────────────
// Methodology in creative/comms/brand work is usually expressed by demonstration
// (a stated visual device, a reference-formula adoption, a modular content
// architecture) rather than by labelling a textbook framework. The cap on
// approach_quality.methodology_clarity must therefore key off "is there
// repeatable structure" rather than "did they name a whitelisted framework".
const CREATIVE_INDUSTRIES = new Set([
  'film, video & audio', 'creative strategy & brand', 'design & experience',
  'advertising & campaigns', 'pr & media relations', 'social & digital content',
  'corporate & internal communications', 'events & experiential', 'content & editorial',
]);
const TECHNICAL_INDUSTRIES = new Set([
  'engineering & technical', 'technology & digital',
  'management consulting', 'strategy & advisory',
  'programme & project management', 'finance & commercial',
  'legal & regulatory',
]);

function countAny(haystack, needles) {
  let n = 0;
  for (const w of needles) if (haystack.includes(w)) n += 1;
  return n;
}

function detectWorkType({ sector, serviceIndustry, text } = {}) {
  if (serviceIndustry) {
    const ind = String(serviceIndustry).toLowerCase();
    if (CREATIVE_INDUSTRIES.has(ind)) return 'creative';
    if (TECHNICAL_INDUSTRIES.has(ind)) return 'technical';
    if (ind.includes('research')) return 'research';
    if (ind.includes('learning') || ind.includes('people & organisational')) return 'learning';
    if (ind.includes('sustainability')) return 'research';
  }

  const lower = (String(text || '').slice(0, 30000) + ' ' + String(sector || '')).toLowerCase();

  const creativeHits = countAny(lower, [
    'tvc', 'master tvc', 'foundation film', 'creative director', 'storyboard',
    'director of photography', ' dop ', 'cinematograph', 'pre-production',
    'post-production', 'edit suite', 'voiceover', 'voice over', 'spoken-word',
    'casting director', 'casting studio', 'gimbal', 'camera package',
    'specialist lighting', 'colour grade', 'music licens', 'campaign',
    'advert', 'spot', 'creative agency', 'mana creative', 'brand campaign',
    'social campaign', 'film & video', 'film and video', 'editorial',
  ]);
  const technicalHits = countAny(lower, [
    'prince2', 'agile delivery', 'scrum', 'safe ', 'devops', 'kubernetes',
    'kafka', 'snowflake', 'data migration', 'microservices', 'api integration',
    'ci/cd', 'sre', 'fhir', 'iso 27001', 'togaf', 'oauth', 'data warehouse',
    'data lake', 'dama-dmbok', 'itil', 'six sigma', 'soc 2',
  ]);
  const researchHits = countAny(lower, [
    'ethnograph', 'mixed methods', 'theory of change', 'longitudinal',
    'qualitative interviews', 'survey methodology', 'rct', 'literature review',
    'meta-analysis', 'sample size', 'meal framework',
  ]);
  const learningHits = countAny(lower, [
    'addie', 'kirkpatrick', '70-20-10', ' sam model', 'learning design',
    'l&d programme', 'instructional design',
  ]);

  const max = Math.max(creativeHits, technicalHits, researchHits, learningHits);
  if (max < 2) return 'unknown';
  if (creativeHits === max) return 'creative';
  if (technicalHits === max) return 'technical';
  if (researchHits === max) return 'research';
  if (learningHits === max) return 'learning';
  return 'unknown';
}

// Returns the work-type-specific methodology-evidence guidance that the LLM
// uses to decide whether to apply the approach_quality.methodology_clarity cap.
// The bar is ALWAYS "is there repeatable, transferable structure" — what
// counts as such varies by domain.
function methodologyEvidenceBlock(workType) {
  if (workType === 'creative') {
    return `WORK TYPE: creative / film / comms / brand / advertising / design / content / events.

Methodology in this domain is usually expressed by demonstration, not labelled
with a textbook name. Treat ANY of the following as evidence of structured
methodology — they count for approach_quality.methodology_clarity scoring just
as PRINCE2 / SAFe would in IT:

  · A named reference work cited as the working formula (e.g. "we follow the
    Apple 'Crazy Ones' formula: spoken-word lead → stripped music → emotive
    pictures").
  · A stated repeatable visual or narrative device explained well enough that
    another team could execute it (e.g. "two camerawork styles intercut —
    'clean' product shots vs 'obscured' subject shots, breaking pattern at
    the reveal moment").
  · A modular content architecture with named pillars and a stated
    relationship to a master deliverable (e.g. "4 foundations × 15-30s
    composited into 1 master TVC × 30-60s, plus an internal-facing variant").
  · A phased production model with day-rate-costed named roles
    (pre-production / production / post-production with Producer, DoP, Editor,
    Casting Director, etc., specific day counts and rates).
  · A named structural framework: three-act, hero's journey, double-diamond,
    RACE, SOSTAC, PESO, jobs-to-be-done, design thinking, observational
    documentary, fly-on-the-wall, cinéma vérité, run-and-gun, Dogme 95,
    director-led, agency model, storyboarding-first, message-house.

Score methodology_clarity 65-80 when one or more of the above is described
clearly enough to be repeatable. The cap STILL TRIGGERS when the document is
genuinely vague — a single "we'll deliver a great campaign tailored to your
brand" paragraph with no device, no architecture, no phasing, no costed
crew. The bar isn't "did they say a textbook word", it's "did they describe
something repeatable".`;
  }

  if (workType === 'technical') {
    return `WORK TYPE: consulting / IT / engineering / data / public-sector / finance / legal.

Methodology in this domain is typically named: PRINCE2, SAFe, Agile/Scrum,
DAMA-DMBOK, TOGAF, FHIR, HIPAA, ISO 27001, ITIL, Six Sigma, Lean, DevOps,
DevSecOps, data-mesh, zero-trust, etc. Apply the methodology cap strictly —
unnamed "our approach" blobs do not count. Proprietary frameworks count
only if they are named AND explained.`;
  }

  if (workType === 'research') {
    return `WORK TYPE: research / insights / evaluation / sustainability.

Methodology in this domain is typically named: ethnographic, grounded-theory,
mixed-methods, MEAL, theory-of-change, RCT design, conjoint, jobs-to-be-done
interviews. Apply the methodology cap; named methods are expected here.`;
  }

  if (workType === 'learning') {
    return `WORK TYPE: learning & development / training / capability-building.

Methodology in this domain is typically named: ADDIE, SAM, 70-20-10,
Kirkpatrick evaluation. Apply the methodology cap; named models are
expected here.`;
  }

  return `WORK TYPE: not classified. Apply the methodology cap strictly — accept
named frameworks from the whitelist or named proprietary frameworks that are
explained. Unnamed "our approach" blobs do not count.`;
}

// ── FULL PROPOSAL ANALYSIS ────────────────────────────────────────────────────
async function analyseProposal(text, rating, userNotes, ctx = {}) {
  ...
  const workType = detectWorkType({
    sector: ctx.sector,
    serviceIndustry: ctx.service_industry,
    text: docText,
  });
  const workTypeBlock = methodologyEvidenceBlock(workType);
  dlog(`[analyseProposal] detected work-type="${workType}" (sector="${ctx.sector || ''}", service_industry="${ctx.service_industry || ''}")`);
...
═══════════════════════════════════════════════════════════════════════════
WORK-TYPE GUIDANCE — calibrate methodology evidence by domain
═══════════════════════════════════════════════════════════════════════════
${workTypeBlock}
...
- Methodology cap: see the WORK-TYPE GUIDANCE block above for what counts
  as structured methodology in this domain. Apply
  approach_quality.methodology_clarity ≤ 50 ONLY when the document does NOT
  exhibit any of the structural evidence enumerated for the relevant work
  type. For creative/comms/brand/design/film work, demonstrated structure
  (named reference formulas, repeatable visual or narrative devices, modular
  content architectures, phased production with day-rate-costed crew) counts
  as methodology — the bar is "is it repeatable", not "did they name a
  textbook". For consulting/IT/research/learning, named frameworks are
  expected and the cap applies strictly when they are absent. In all cases,
  unnamed "our approach" blobs with no device, architecture, or phasing
  detail still trigger the cap.
...
score is too high.`;
  ...
  "methodologies": ["named methodologies AND demonstrated structural devices that count per the WORK-TYPE GUIDANCE block — e.g. 'PRINCE2', 'SAFe', or for creative work 'four-pillar content architecture composited into master TVC', 'two-style intercut camerawork device with reveal-at-thank-you', 'Crazy Ones reference formula (spoken-word lead → stripped music → emotive pictures)'. Empty array only if neither named frameworks nor repeatable devices are present."],
  ...
  "approach_quality": {
    ...
    "overall_rationale": "OPTIONAL. Include only when overall_score diverges from the sub-score mean by more than ±8. Explain why in one sentence.",
    "methodology_clarity": "integer 0-100. Score against the WORK-TYPE GUIDANCE block above — demonstrated structural devices count as methodology in creative/comms/brand/design/film work; named frameworks are expected in consulting/IT/research/learning.",
    "methodology_notes": "name the methodology(ies) or device(s) — could be a textbook framework (PRINCE2, double-diamond), a named reference formula adopted, a stated repeatable visual/narrative device, a modular content architecture, or a phased production model with costed roles. Cite the actual text. State explicitly if the document describes only vague 'our approach' language.",
...
Return the final JSON after </thinking>. Return ONLY valid JSON in your final answer (no prose, no code fences).`;
  ...
  try {
    if (hasOpenAI()) {
      // Tier 1 — flagship reasoning. Hard caps + bias rules + structured
      // output schema benefit from gpt-5.5-pro's stricter rule-following.
      const raw = await openaiGenerate(systemSuffix, userPrompt, 6000, 'analyseProposal');
...
❌ DO NOT INVENT requirements that have no basis in the document or
  ...
  "budget_range": { "low": "lower bound number if stated, e.g. 500000", "high": "upper bound number if stated, e.g. 1200000", "raw": "verbatim quote from RFP, e.g. '$500,000 – $1.2M'" },
  "currency": "ISO currency code stated or implied by the RFP — USD, GBP, EUR, AED, etc. Default null if not deducible.",
  "timeline": "headline timeline statement from RFP, e.g. '12 weeks from award to launch'",
  "timeline_weeks": "integer total weeks if a single window is stated; null otherwise",
  "deliverables": ["EVERY deliverable named in the RFP — exact phrasing where possible. e.g. 'Creative concept (minimum 2 routes)', 'Script + storyboard for hero film'. These are gating items: each must be addressed in the response."],
  "background_priorities": ["Strategic priorities or repositioning themes the client has named in their background context. e.g. 'modern, human, premium repositioning', 'sustainability investment', 'upgraded cabin experience'. These are not requirements but they are signals the response must engage with."],
...
═══════════════════════════════════════════════════════════════════════════
NON-INVENTION RULE FOR CREDENTIALS (suggested_person_cv)
═══════════════════════════════════════════════════════════════════════════

The suggested_person_cv field MUST quote a real credential found in the
team member's data:
  • A literal item from their certifications field, OR
  • A specific specialism explicitly listed in their stated_specialisms,
    OR
  • A concrete past project / role from their cv_extracted background.

DO NOT INVENT a credential that fits the gap. Specifically forbidden:
  ✗ "Holds a Certificate in Advanced X" when no such cert is on file
  ✗ "Certified Y Professional" without it being in certifications
  ✗ Any qualification phrase paraphrased from the gap title rather
    than drawn from the supplied team data

If no real credential, specialism, or background item matches, set
suggested_person_cv to null and add a short suggested_action like
"Confirm this team member's relevant credentials before submission".
The user can audit fabrication risk; you cannot.
  ...
  "snippets": [
    {
      ...
      "use_case": "executive summary|approach section|credibility|risk|value proposition|methodology|delivery confidence|governance",
...
DELIVERY-CONFIDENCE / GOVERNANCE CATEGORY (important — evaluators on
high-value creative & production bids score this explicitly):

Actively hunt for sentences from past wins that demonstrate
production discipline, governance, or delivery control — even if
they're buried in methodology sections. Examples of what qualifies:
  • explicit phase structure with named gates or approvals
  • risk register / contingency / mitigation language
  • decision rights and escalation paths
  • named governance roles ("Project Lead accountable for…")
  • cost & scope change handling mechanics

If a past won proposal contains strong delivery-confidence language,
pull at least 1-2 such snippets and tag them use_case="delivery
confidence" or "governance". Do NOT pad — only include if the
language is specific and reusable. Evaluators on six-figure creative
bids penalise responses that feel creatively strong but operationally
vague; this category exists to close that specific gap.`;
...
${orgBlock}`;
  ...
  // Flash-preferred (Wave 6 Phase 3): mechanical relevance call, runs
  // ~5× per scan, identical quality on Flash. Saves ~$0.06/scan.
  const useFlashFirst = preferFlashFor('explainMatch');
  const geminiPrompt = `Explain why this past proposal is relevant to this RFP. Be specific and honest. Return ONLY valid JSON.\n\nSchema:\n${schema}\n\n${context}`;
  const oaiUserPrompt = `${context}\n\nExplain why this past proposal is or isn't relevant.\n\nReturn ONLY valid JSON:\n${schema}`;

  if (useFlashFirst) {
    try {
      const raw = await geminiGenerate(geminiPrompt, true);
      const parsed = safeJSON(raw);
      if (parsed) return parsed;
    } catch (e) { console.error('Gemini explainMatch failed, falling back to OpenAI:', e.message); }
  }

  try {
    if (hasOpenAI()) {
      const raw = await openaiGenerate(systemSuffix, oaiUserPrompt);
      return safeJSON(raw);
    }
  } catch (e) {
...
Return ONLY valid JSON.`;
  ...
  // Flash-preferred (Wave 6 Phase 3): mechanical entity extraction —
  // string-matching task, identical quality on Flash, saves ~$0.014/scan.
  const useFlashFirst = preferFlashFor('extractSearchableEntities');
  const oaiSystem = 'You are a precise procurement analyst. Extract only specific named entities, never generic descriptions.';
  ...
  if (useFlashFirst) {
    try {
      ...
      const parsed = safeJSON(raw);
      if (parsed) return parsed;
    } catch (e) { console.error('Gemini extractSearchableEntities failed, falling back to OpenAI:', e.message); }
  }

  try {
    if (hasOpenAI()) {
      const raw = await openaiGenerate(oaiSystem, prompt, 800);
      return safeJSON(raw);
    }
...
Return ONLY valid JSON after </thinking>.`;
  ...
  // Flash-preferred (Wave 6 Phase 3): scoring/categorisation task.
  // Saves ~$0.037/scan. URL hallucination is filtered downstream so
  // the small Flash quality gap on relevance scoring is acceptable.
  const useFlashFirst = preferFlashFor('scoreAndCategoriseMarketContext');
  const validUrls = new Set(searchResults.map(r => r.url));
  const filterValid = items => items
  ...
  if (useFlashFirst) {
    try {
      const raw = await geminiGenerate(prompt, true);
      const parsed = safeJSON(raw);
      if (parsed?.items) return filterValid(parsed.items);
    } catch (e) { console.error('Gemini scoreAndCategorise failed, falling back to OpenAI:', e.message); }
  }
...
// Has the RFP yielded any genuine procurement entities? Programmes,
// frameworks, issuing bodies, and regulations are the "this is a gov
// procurement bid" signals. technology_classes alone is too weak — a
// creative brief might list "social media management" or "video
// production" as tech, but that doesn't make it procurement.
function hasProcurementSignals(entities) {
  if (!entities) return false;
  return (
    (entities.programmes || []).length > 0 ||
    (entities.frameworks || []).length > 0 ||
    (entities.issuing_bodies || []).length > 0 ||
    (entities.regulations || []).length > 0
  );
}

// Build queries for non-procurement RFPs (creative, brand, marketing,
// film, commercial). The signal here is the client itself, the client's
// industry vertical, and the brief's themes — not procurement vehicles.
function buildCreativeQueries(rfpData) {
  const queries = [];
  const client = rfpData?.client && rfpData.client !== 'Unknown' ? rfpData.client : '';
  const sector = rfpData?.sector || '';
  const themes = (rfpData?.key_themes || []).slice(0, 2);
  const clientIndustry = rfpData?.client_industry || '';

  // Tier 1 — the client itself. News about who they are and what they're doing.
  if (client) {
    queries.push({ tier: 1, query: `"${client}"`, label: 'client_news' });
  }

  // Tier 2 — client + industry vertical. Stories about their world.
  if (client && clientIndustry) {
    queries.push({ tier: 2, query: `"${client}" ${clientIndustry}`, label: 'client_industry' });
  } else if (client && sector) {
    queries.push({ tier: 2, query: `"${client}" ${sector}`, label: 'client_sector' });
  }

  // Tier 3 — sector trends. Broader environmental shifts in the brief's domain.
  if (sector) {
    queries.push({ tier: 3, query: `${sector} trends ${new Date().getFullYear()}`, label: 'sector_trend' });
  }

  // Tier 4 — themes. The brief's specific creative or strategic angle.
  themes.forEach(theme => {
    if (theme && theme.length > 3) {
      queries.push({ tier: 4, query: `${theme} ${sector || ''}`.trim(), label: 'theme' });
    }
  });
...
// Score articles for non-procurement RFPs. Different signals matter:
// the client by name, their industry, brand/agency competitor moves,
// trend stories that connect to the brief. Threshold lowered to 40
// because the bar to "useful" is different — a brand campaign reference
// is genuinely useful even if it's not a programme announcement.
async function scoreCreativeContext(searchResults, rfpData) {
  if (!searchResults.length) return [];
  ...
  const systemSuffix = `You are a creative strategist scoring news articles for relevance to a brand, marketing, film, or creative-services brief.

This is NOT a government procurement bid. The client is a commercial / cultural / brand organisation. The relevance bar is "would this story shape what the agency proposes, references, or pitches against?".

═══════════════════════════════════════════════════════════════════════════
SCORING RUBRIC (out of 100)
═══════════════════════════════════════════════════════════════════════════
  +30  Mentions the specific client by name (their news, leadership, campaigns, launches)
  +20  About the client's industry vertical / what they actually sell or do
  +15  Competitor agency win, brand campaign in the same vertical, M&A in the supplier base
  +15  Trend story directly relevant to the brief's themes or creative angle
  +10  From a respected trade press source for this domain (Campaign, AdWeek, The Drum, AdAge, Digiday, Variety, etc.)
  -25  Obviously off-topic (different country with no relevance, hobbyist content, unrelated commodity)
  -15  Generic broad-industry filler with no specific tie to the client or brief

THRESHOLD: any article scoring below 40 must be REJECTED. Better 0 articles than padding.

═══════════════════════════════════════════════════════════════════════════
CATEGORIES
═══════════════════════════════════════════════════════════════════════════
For each surviving article, assign ONE category:
  - "client_news"   = news about the client themselves
  - "sector_trend"  = trends or shifts in the client's industry
  - "competitive"   = competitor agencies, comparable brand campaigns, supplier-side moves
  - "brief_context" = stories that directly inform the brief's specific themes or angle

═══════════════════════════════════════════════════════════════════════════
WHAT TO RETURN PER ARTICLE
═══════════════════════════════════════════════════════════════════════════
For each survivor:
  - relevance_score (40-100, calibrated)
  - category (one of the four above)
  - why_it_matters: one sentence on how this affects the creative pitch
  - where_to_use_in_bid: which section of the bid this should inform
  - tone_supported: e.g. "credibility", "cultural relevance", "differentiation", "urgency"
...
Client: ${rfpData.client || 'Unknown'}
Sector: ${rfpData.sector || 'Unknown'}
Client industry: ${rfpData.client_industry || 'unspecified'}
Themes: ${(rfpData.key_themes || []).join(', ') || 'none'}
...
Then return JSON with the survivors only (score >= 40). Better 0 articles
than 6 irrelevant ones.
  ...
  "items": [
    {
      ...
      "snippet": "1-2 sentence summary",
      "relevance_score": 65,
      "category": "client_news | sector_trend | competitive | brief_context",
      "why_it_matters": "one specific sentence",
      "where_to_use_in_bid": "executive summary | creative approach | client understanding | competitive positioning | cultural relevance",
      "tone_supported": "credibility | cultural relevance | differentiation | urgency | reassurance"
    }
  ]
}
...
Return ONLY valid JSON after </thinking>.`;
  ...
  // Map creative categories back to the existing 4 UI categories so the
  // UI keeps grouping cleanly without a frontend change. client_news →
  // buyer (it's about the buyer in this context), sector_trend →
  // tech_reg (broader environmental shift), brief_context → programme
  // (specific opportunity context).
  const CREATIVE_TO_UI = {
    client_news: 'buyer',
    sector_trend: 'tech_reg',
    competitive: 'competitive',
    brief_context: 'programme',
  };

  const validUrls = new Set(searchResults.map(r => r.url));
  const filterValid = items => items
    .filter(it => it.url && validUrls.has(it.url))
    .filter(it => (it.relevance_score || 0) >= 40)
    .map(it => ({ ...it, category: CREATIVE_TO_UI[it.category] || it.category, mode: 'creative' }));
  ...
  if (useFlashFirst) {
    try {
      ...
      const parsed = safeJSON(raw);
      if (parsed?.items) return filterValid(parsed.items);
    } catch (e) { console.error('Gemini scoreCreative failed, falling back to OpenAI:', e.message); }
  }
  ...
  } catch (e) { console.error('scoreCreative OpenAI:', e.message); }

  try {
    const raw = await geminiGenerate(prompt, true);
    const parsed = safeJSON(raw);
    if (parsed?.items) return filterValid(parsed.items);
  } catch (e) { console.error('scoreCreative Gemini:', e.message); }
...
async function getIndustryNews(rfpData, rfpText) {
  if (!process.env.BRAVE_SEARCH_KEY) {
    dlog('BRAVE_SEARCH_KEY not set — market context tab will be empty');
    return [];
  }
  ...
  // Branch on whether this looks like a procurement bid or a creative
  // brief. Procurement signals = programmes, frameworks, issuing bodies,
  // regulations. Anything else routes to the creative pipeline (client +
  // sector trends), which has different queries, scoring, and threshold.
  const isProcurement = hasProcurementSignals(entities);
  dlog(`[market-context] mode=${isProcurement ? 'procurement' : 'creative'} client="${rfpData?.client || '?'}" sector="${rfpData?.sector || '?'}"`);

  if (isProcurement) {
    const queries = buildTieredQueries(entities, rfpData);
    ...
    return items.map(it => ({ ...it, mode: 'procurement' })).sort((a, b) => {
  ...
  // Creative path — different queries, different scoring rubric, lower threshold.
  const queries = buildCreativeQueries(rfpData);
  if (queries.length === 0) return [];
  const searchResults = await searchBraveForQueries(queries);
  if (searchResults.length === 0) return [];
  const items = await scoreCreativeContext(searchResults, rfpData);
...
Start with "For this bid:" and be direct.`;
  ...
  // OpenAI first (consistent with the other deep-judgment calls), Gemini
  // fallback. Returns plain prose, not JSON. The previous Gemini-only path
  // dropped silently on quota / overload and the empty string was indistinguishable
  // from "no advice generated", so failures were invisible. Both attempts now
  // throw on failure so the caller can log the real error.
  const systemSuffix = `You are a senior bid strategist giving narrative structure advice for a single bid. Output plain prose, no JSON. Be specific.`;
  if (hasOpenAI()) {
    try {
      return await openaiGenerateText(systemSuffix, prompt, 1500, 'getNarrativeAdvice');
    } catch (e) {
      console.error('OpenAI getNarrativeAdvice failed, falling back to Gemini:', e.message);
    }
  }
  try {
    return await geminiGenerate(prompt, false, 'getNarrativeAdvice');
  } catch (e) {
    console.error('getNarrativeAdvice Gemini fallback failed:', e.message);
    throw e;
  }
}
...
// ── VISION FIELD EXTRACTOR ────────────────────────────────────────────────────
// Used when text extraction misses fields that live on cover pages, in
// coloured/formatted tables, or in image-based PDFs (logos, scans).
// Renders the first 3 pages to JPEG via pdftoppm and asks gpt-4o-mini.
//
// `fields` is one of: ['contract_value'], ['client', 'sector'], or any
// combination — the function only asks the model for what's needed so
// the prompt stays focused.
// Cached `pdftoppm` availability check. Logged once on startup, then
// vision extraction silently no-ops on environments where the binary
// is missing — e.g. fresh Railway containers before poppler-utils was
// added to the build. Without this we flood logs with "Command failed".
let _pdftoppmAvailable = null;
function pdftoppmAvailable() {
  if (_pdftoppmAvailable !== null) return _pdftoppmAvailable;
  try {
    require('child_process').execSync('which pdftoppm', { stdio: 'pipe' });
    _pdftoppmAvailable = true;
  } catch {
    _pdftoppmAvailable = false;
    console.warn('pdftoppm not installed — vision-based field extraction disabled. Add poppler-utils to the build.');
  }
  return _pdftoppmAvailable;
}

async function extractFieldsFromImages(filePath, fields = ['contract_value']) {
  if (!hasOpenAI()) return null;
  if (!pdftoppmAvailable()) return null;
  const fs = require('fs');
  const path = require('path');
  ...
  const tmpDir = path.join(os.tmpdir(), `piq_vision_${Date.now()}_${Math.random().toString(36).slice(2,8)}`);
  try {
    ...
    if (fields.includes('contract_value')) {
      fieldInstructions.push(`- Total contract value: total fee, budget, project cost, or any clear monetary total. Ignore day rates and line items unless they're the only values shown. Convert £250K → 250000, $1.2M → 1200000.`);
      schemaFields.push(`"contract_value": 250000`, `"currency": "GBP"`);
    }
    if (fields.includes('client')) {
      fieldInstructions.push(`- Client organisation: look for "prepared for", "submitted to", addressee on cover page, client logo, recipient header.`);
      schemaFields.push(`"client": "client organisation name or null"`);
    }
    if (fields.includes('sector')) {
      fieldInstructions.push(`- Sector: industry the client operates in. Use a specific named sector — Energy & Utilities, Education, Manufacturing, Construction, Charity, Local Government, etc. Do not default to "Other"; pick the closest specific sector.`);
      schemaFields.push(`"sector": "specific sector name"`);
    }
    schemaFields.push(`"confidence": "high|medium|low"`, `"found_in": "brief description of where you found these e.g. cover page header"`);

    const visionModel = process.env.OPENAI_VISION_MODEL || 'gpt-4o-mini';
    const resp = await openaiEnqueue(() => _openaiCallWithRetry({
      model: visionModel,
      ...tokenLimitParam(visionModel, 400),
      messages: [{
        role: 'user',
        content: [
          ...imageContent,
          {
            type: 'text',
            text: `Look at these proposal/tender document pages and extract:
...
If a field is not findable, set it to null. Do not guess.`,
          },
        ],
      }],
    }, 'extractFieldsFromImages'));

    const raw = resp.choices[0]?.message?.content || '';
    return safeJSON(raw) || null;

  } catch (e) {
    console.error('extractFieldsFromImages:', e.message);
    return null;
  } finally {
    try {
...
// Back-compat alias — old call sites still expect this name.
const extractPricingFromImages = (filePath) => extractFieldsFromImages(filePath, ['contract_value']);
...
- Sector: pick the specific industry the client operates in — Government & Public Sector, Healthcare, Education, Energy & Utilities, Manufacturing, Construction, Financial Services, Charity & Third Sector, Technology, Transport, Retail, Aerospace & Defence, Film & Media, Creative & Production, etc. Use whatever sector best fits — do NOT default to "Other" unless it really doesn't fit any named industry.
...
Return ONLY valid JSON:
{
  ...
  "sector": "specific sector name (free text — pick the best match for this client) or null if genuinely unclear",
  "contract_value": "numeric only e.g. 250000, or null",
  "currency": "GBP|USD|EUR|AUD|CAD|AED|other ISO code or null",
  "project_type": "specific project type (free text) — Digital Transformation, ERP Integration, Cybersecurity, Film Production, Brand Campaign, etc., or null",
  ...
  try {
    ...
    if (parsed.sector && parsed.sector.toLowerCase() !== 'other') extracted.sector = parsed.sector;
    ...
    // Vision fallback: when text extraction missed contract value, client,
    // or sector AND confidence wasn't high, render the cover/first pages
    // and ask vision to find them. Cover-page logos, formatted tables, and
    // image-based PDFs all leak past pdf-parse.
    const visionFields = [];
    if (!extracted.contract_value) visionFields.push('contract_value');
    if (!extracted.client) visionFields.push('client');
    if (!extracted.sector) visionFields.push('sector');
    if (visionFields.length && filePath && parsed.confidence !== 'high') {
      try {
        const v = await extractFieldsFromImages(filePath, visionFields);
        if (v) {
          if (visionFields.includes('contract_value') && v.contract_value) {
            extracted.contract_value = String(v.contract_value).replace(/[^0-9.]/g, '');
            if (v.currency) extracted.currency = v.currency;
          }
          if (visionFields.includes('client') && v.client) extracted.client = v.client;
          if (visionFields.includes('sector') && v.sector && v.sector.toLowerCase() !== 'other') {
            extracted.sector = v.sector;
          }
          console.log(`Vision extraction filled: ${visionFields.filter(f => extracted[f === 'contract_value' ? 'contract_value' : f]).join(', ')} (${v.found_in || 'unspecified'})`);
        }
      } catch (e) {
...
async function generateApproachAndBudget(rfpData, matchedProposals, teamSuggestions, rateCardRoles = []) {
  ...
  // Anchor to RFP-stated budget range if present — this is the client's own
  // ceiling/floor and must be respected unless we have a concrete reason to
  // propose outside it. Without this, the model would invent a range blind
  // to what the client already said.
  const rfpBudgetRaw = rfpData?.budget_range?.raw
    || (rfpData?.budget_range?.low && rfpData?.budget_range?.high
        ? `${rfpData.currency || ''} ${Number(rfpData.budget_range.low).toLocaleString()}–${Number(rfpData.budget_range.high).toLocaleString()}`
        : null);
  const rfpCurrency = rfpData?.currency || null;
  const rfpDeliverables = Array.isArray(rfpData?.deliverables) ? rfpData.deliverables : [];
  const rfpImplicit = Array.isArray(rfpData?.implicit_requirements)
    ? rfpData.implicit_requirements.map(r => r?.text || r).filter(Boolean).slice(0, 5)
    : [];
...
Budgets must be grounded in: (a) the RFP's stated budget range if given, (b) the specific deliverables being priced, (c) industry norms for this type of work, (d) the actual day rates / rate card available.
Do not generate generic phases — every phase must have a clear rationale from the matched work.

BUDGET DISCIPLINE:
- If the RFP states a budget range, propose INSIDE that range by default. If proposing outside, justify in one sentence (explicit scope premium or deliberate lean-in).
- Quote in the RFP's stated currency. If internal rates are in a different currency, convert and state the FX assumption.
- Produce a per-workstream BREAKDOWN (strategy / pre-production / production / post / management — or whatever the deliverables demand). Each line must state its own cost drivers (e.g. "2 countries, 5-day shoot, premium DOP").
- If rate card / team doesn't cover the deliverables (e.g. film shoot with no production roles on rate card), explicitly say so and use industry benchmark reasoning — don't fabricate role-rate maths.`;
  ...
  "indicative_budget": {
    ...
    "high": 420000,
    "currency": "GBP",
    "basis": "2-3 sentences. State whether this sits inside/outside the RFP's stated range and why. Name the top 2-3 cost drivers specific to this brief (not generic).",
    "confidence": "high|medium|low",
    "breakdown": [
      { "workstream": "e.g. Strategy & creative development", "low": 80000, "high": 180000, "drivers": "one sentence on what moves this line up or down for this specific brief" }
    ],
    "rfp_range_alignment": "inside|below|above|no_range_stated — one phrase on how this relates to the RFP's stated range"
  },
  ...
  const budgetAnchor = rfpBudgetRaw
    ? `RFP-STATED BUDGET RANGE: ${rfpBudgetRaw}${rfpCurrency ? ` (${rfpCurrency})` : ''}
→ Propose inside this range by default. Quote in ${rfpCurrency || 'the RFP currency'}. If proposing outside, justify in the basis field.`
    : `RFP did not state a budget range. Propose a defensible range based on industry norms + deliverable complexity, and set rfp_range_alignment to "no_range_stated".`;

  const deliverableAnchor = rfpDeliverables.length > 0
    ? `DELIVERABLES TO PRICE (each is a cost driver — your breakdown must cover them):
${rfpDeliverables.map(d => `  · ${d}`).join('\n')}`
    : 'Deliverables not extracted — infer the main cost drivers from the scope/themes.';

  const implicitAnchor = rfpImplicit.length > 0
    ? `IMPLICIT COST DRIVERS (evaluators will expect these priced in even if not called out):
${rfpImplicit.map(r => `  · ${r}`).join('\n')}`
    : '';
...
Budget guidance: Use rate card role costs where they cover the scope. Named team members override rate card costs where available. When the deliverables fall outside rate card coverage (e.g. film production, shoot crew, location costs, media/licensing, post-production grade — none of which typically appear on a consulting rate card), state that explicitly in the basis and use industry benchmark reasoning instead of fabricated role-rate maths.`;
...
[{"id":"proposal_id","rank":1,"relevance":"high|medium|low","reason":"one specific sentence why this is or isn't a strong practical match"}]`;
  ...
  // Flash-preferred (Wave 6 Phase 3): re-ranking is judgement on small
  // structured input — Flash matches gpt-4o for this. Saves ~$0.047/scan.
  const useFlashFirst = preferFlashFor('reRankProposals');
  ...
  if (useFlashFirst) {
    try {
      ...
      const result = buildResult(safeJSON(raw));
      if (result) return result;
    } catch (e) { console.error('Gemini reRankProposals failed, falling back to OpenAI:', e.message); }
  }
...
// ── BID/NO-BID SCORING ────────────────────────────────────────────────────────
// winRate can be a plain number (legacy) OR an object
// { rate, basis, pool_size } from computeWinRate(). The object form lets
// the rationale label the comparison basis ("sector" vs "service") and
// skip the label entirely when the pool is too small to be meaningful.
async function scoreBid(rfpData, matches, gaps, winRate, teamAvailable) {
  ...
  // Accept both legacy number form and the new { rate, basis, pool_size }
  // object form. Basis lets the rationale label the pool honestly.
  const winRateNum = typeof winRate === 'number' ? winRate : (winRate?.rate || 0);
  const winRateBasis = typeof winRate === 'object' ? (winRate?.basis || 'none') : 'sector';
  ...
  const winRateScore = winRateNum > 0 ? Math.min(100, winRateNum * 1.5) : 50;
  ...
  if (winRateNum > 0) {
    const label = winRateBasis === 'service_industry'
      ? `${winRateNum}% win rate on this type of work`
      : winRateBasis === 'sector'
        ? `${winRateNum}% sector win rate`
        : `${winRateNum}% win rate`;
...
async function generateSectionDraft(sectionName, sectionDescription, rfpData, matchedProposals, winStrategy, winningLanguage, executiveBrief = null, orgProfile = null, opts = {}) {
  const { gaps = [], suggestedApproach = null, narrativeAdvice = '', writingInsights = [], team = [], contract = null } = opts || {};
  ...
  // Route winning-language snippets to the section they were tagged for.
  // Falls back to all snippets when no use_case match is found, so a
  // section with no targeted snippets isn't left empty-handed.
  const targetedUseCases = Array.isArray(contract?.snippet_use_cases) ? contract.snippet_use_cases.map(s => s.toLowerCase()) : [];
  const allSnippets = Array.isArray(winningLanguage) ? winningLanguage : [];
  const sectionSnippets = targetedUseCases.length > 0
    ? allSnippets.filter(s => {
        const uc = (s.use_case || '').toLowerCase();
        return targetedUseCases.some(t => uc.includes(t) || t.includes(uc));
      })
    : [];
  const snippetsToShow = sectionSnippets.length > 0 ? sectionSnippets : allSnippets.slice(0, 4);
  const langContext = snippetsToShow.slice(0, 6).map((s, i) => {
    const adapted = s.adapted || s.text || '';
    const why = s.why_it_works ? `\n   WHY IT WORKS: ${s.why_it_works}` : '';
    const how = s.how_to_adapt ? `\n   HOW TO ADAPT: ${s.how_to_adapt}` : '';
    const useIn = s.use_case ? ` (${s.use_case})` : '';
    return `[L${i + 1}]${useIn} "${adapted}"${why}${how}`;
  }).join('\n\n') || 'No winning language available for this section';
...
${(() => {
  // REQUIRED INPUTS for this section. The framing is firmer than before:
  // the upstream win strategy, suggested approach, gaps, narrative advice,
  // and writing insights are not optional context — they are the inputs
  // the bid team paid to produce. The model must apply them where
  // relevant to this section.
  const bits = [];

  // ── Win strategy: the most underused signal in the previous version
  if (winStrategy && typeof winStrategy === 'object') {
    bits.push('WIN STRATEGY (apply where this section can carry it):');
    if (winStrategy.opening_narrative) {
      bits.push(`  · Opening narrative (adapt and use in Executive Summary; thread its angle elsewhere): "${winStrategy.opening_narrative}"`);
    }
    if (winStrategy.differentiator_angle) {
      bits.push(`  · Differentiator angle (must thread through Approach + Why-Us + Executive Summary): ${winStrategy.differentiator_angle}`);
    }
    const tops = Array.isArray(winStrategy.top_priorities) ? winStrategy.top_priorities : [];
    if (tops.length > 0) {
      bits.push('  · Top priorities (each MUST be addressed in the section that owns it):');
      tops.forEach(p => {
        const what = p.priority || p.title || '';
        const why = p.why_it_matters || p.rationale || '';
        const ev = p.evidence ? ` (evidence: ${p.evidence})` : '';
        bits.push(`      - ${what}${why ? ` — why: ${why}` : ''}${ev}`);
      });
    }
    const risks = Array.isArray(winStrategy.risks_to_mitigate) ? winStrategy.risks_to_mitigate : [];
    if (risks.length > 0) {
      bits.push('  · Risks to mitigate (the QA / risk section MUST address each; other sections should pre-empt where natural):');
      risks.forEach(r => {
        const what = r.risk || r.title || '';
        const fix = r.mitigation || r.fix || '';
        bits.push(`      - ${what}${fix ? ` — mitigation: ${fix}` : ''}`);
      });
    }
    const emphasise = Array.isArray(winStrategy.emphasise_these || winStrategy.emphasise) ? (winStrategy.emphasise_these || winStrategy.emphasise) : [];
    if (emphasise.length > 0) {
      bits.push(`  · EMPHASISE: ${emphasise.join(' | ')}`);
    }
    const avoid = Array.isArray(winStrategy.avoid_these || winStrategy.avoid) ? (winStrategy.avoid_these || winStrategy.avoid) : [];
    if (avoid.length > 0) {
      bits.push(`  · AVOID: ${avoid.join(' | ')}`);
    }
  }

  // ── Suggested approach: full phases, no slicing
  if (suggestedApproach && typeof suggestedApproach === 'object') {
    const phases = Array.isArray(suggestedApproach.suggested_phases) ? suggestedApproach.suggested_phases : [];
    if (phases.length > 0) {
      bits.push('', 'SUGGESTED APPROACH — full phase plan (the Approach section MUST reproduce every phase substantively, not as one-liners):');
      phases.forEach((ph, i) => {
        const name = ph.phase_name || ph.name || `Phase ${i + 1}`;
        const dur = ph.duration || ph.duration_weeks || '';
        const rationale = ph.rationale || '';
        const activities = Array.isArray(ph.activities) ? ph.activities : [];
        const teamRoles = Array.isArray(ph.team_roles) ? ph.team_roles : [];
        const deliverables = Array.isArray(ph.deliverables) ? ph.deliverables : [];
        bits.push(`  Phase ${i + 1}: ${name}${dur ? ` (${dur})` : ''}`);
        if (rationale) bits.push(`    Rationale: ${rationale}`);
        if (activities.length > 0) {
          bits.push('    Activities:');
          activities.forEach(a => bits.push(`      · ${a}`));
        }
        if (deliverables.length > 0) bits.push(`    Deliverables: ${deliverables.join('; ')}`);
        if (teamRoles.length > 0) bits.push(`    Team on this phase: ${teamRoles.join(', ')}`);
      });
    }
    ...
    // ── Timeline reconciliation: sum the phase weeks against the RFP's
    // stated timeline. If they exceed, the Approach section MUST compress.
    const phasesForSum = phases || [];
    const totalWeeks = phasesForSum.reduce((acc, ph) => {
      const m = String(ph.duration || ph.duration_weeks || '').match(/(\d+)/);
      return acc + (m ? parseInt(m[1], 10) : 0);
    }, 0);
    const rfpWeeks = Number(rfpData?.timeline_weeks) || 0;
    if (totalWeeks > 0 && rfpWeeks > 0) {
      if (totalWeeks > rfpWeeks) {
        bits.push('', `⚠ TIMELINE OVER-RUN: suggested phases sum to ${totalWeeks} weeks but the RFP stated timeline is ${rfpWeeks} weeks. The Approach section MUST compress phases to fit within ${rfpWeeks} weeks (overlap, parallelise, or trim) — do not propose a plan that exceeds the stated window. State the compressed durations explicitly.`);
      } else {
        bits.push('', `Timeline check: suggested phases (${totalWeeks} weeks) fit within RFP timeline (${rfpWeeks} weeks).`);
      }
    } else if (totalWeeks > 0) {
      bits.push('', `Suggested timeline: ${totalWeeks} weeks total across ${phasesForSum.length} phases.`);
    }

    // ── Budget reconciliation: align the indicative budget to the RFP's
    // stated currency and range. If outside, the Commercial section must
    // either align or explicitly justify.
    const budget = suggestedApproach.indicative_budget;
    if (budget && typeof budget === 'object') {
      const ourCur = budget.currency || 'GBP';
      const fmt = (v, cur) => v ? `${cur} ${Number(v).toLocaleString()}` : null;
      const parts = [
        fmt(budget.low, ourCur) && `LOW ${fmt(budget.low, ourCur)}`,
        fmt(budget.mid, ourCur) && `MID ${fmt(budget.mid, ourCur)}`,
        fmt(budget.high, ourCur) && `HIGH ${fmt(budget.high, ourCur)}`,
      ].filter(Boolean);
      if (parts.length > 0) {
        bits.push('', `INDICATIVE BUDGET (Commercial section MUST quote real numbers, not "tiered approach"): ${parts.join(' · ')}`);
        if (budget.basis) bits.push(`  Basis: ${budget.basis}`);
      }
      // Per-workstream breakdown — when present, Commercial section should
      // mirror this structure rather than quoting a single flat total.
      const breakdown = Array.isArray(budget.breakdown) ? budget.breakdown : [];
      if (breakdown.length > 0) {
        bits.push(`  Workstream breakdown (Commercial section MUST present this split, not a single flat total):`);
        breakdown.forEach(b => {
          const lo = fmt(b.low, ourCur);
          const hi = fmt(b.high, ourCur);
          const range = (lo && hi) ? `${lo}–${hi}` : (lo || hi || '');
          const driver = b.drivers ? ` — ${b.drivers}` : '';
          bits.push(`    · ${b.workstream || 'Workstream'}: ${range}${driver}`);
        });
      }
      if (budget.rfp_range_alignment && budget.rfp_range_alignment !== 'no_range_stated') {
        bits.push(`  RFP range alignment: ${budget.rfp_range_alignment} — Commercial section must acknowledge this explicitly.`);
      }
      // Reconciliation against RFP's stated range + currency
      const rfpCur = rfpData?.currency || null;
      const rfpRange = rfpData?.budget_range || null;
      if (rfpCur && rfpCur !== ourCur) {
        bits.push(`  ⚠ CURRENCY MISMATCH: RFP states ${rfpCur} but our internal budget is in ${ourCur}. The Commercial section MUST quote in ${rfpCur} (the RFP currency). Convert internal day rates and state the FX assumption.`);
      }
      if (rfpRange && (rfpRange.low || rfpRange.high)) {
        const rfpLo = Number(rfpRange.low) || 0;
        const rfpHi = Number(rfpRange.high) || 0;
        const ourMid = Number(budget.mid) || Number(budget.high) || Number(budget.low) || 0;
        if (rfpHi > 0 && ourMid > 0 && ourMid < rfpLo * 0.7) {
          bits.push(`  ⚠ BUDGET MATERIALLY BELOW RFP RANGE: RFP states ${rfpRange.raw || `${rfpCur} ${rfpLo.toLocaleString()}–${rfpHi.toLocaleString()}`}. Internal estimate (~${ourMid.toLocaleString()}) is well below the RFP's stated low. The Commercial section MUST either (a) propose within the RFP's range (justified by scope) or (b) explicitly acknowledge proposing below and explain why. Do NOT silently quote our internal lower number — that signals we did not read the brief.`);
        } else if (rfpHi > 0 && ourMid > rfpHi * 1.3) {
          bits.push(`  ⚠ BUDGET MATERIALLY ABOVE RFP RANGE. The Commercial section MUST acknowledge proposing above the stated range and justify the scope premium.`);
        }
      }
    } else if (rfpData?.budget_range && (rfpData.budget_range.low || rfpData.budget_range.high)) {
      bits.push('', `INDICATIVE BUDGET: no internal estimate available — the Commercial section MUST quote within the RFP's stated range (${rfpData.budget_range.raw || `${rfpData.currency || ''} ${rfpData.budget_range.low}–${rfpData.budget_range.high}`}) using [EVIDENCE NEEDED] markers for any unknowns.`);
    }
    const keyRisks = Array.isArray(suggestedApproach.key_risks) ? suggestedApproach.key_risks : [];
    if (keyRisks.length > 0) {
      bits.push(`  Key delivery risks: ${keyRisks.join(' | ')}`);
    }
    const diffs = Array.isArray(suggestedApproach.differentiators_to_emphasise) ? suggestedApproach.differentiators_to_emphasise : [];
    if (diffs.length > 0) {
      bits.push(`  Differentiators to emphasise: ${diffs.join(' | ')}`);
    }
  }

  // ── High-priority gaps (existing behaviour, tightened framing)
  const topGaps = Array.isArray(gaps) ? gaps.filter(g => g?.priority === 'high' || g?.priority === 'High') : [];
  if (topGaps.length > 0) {
    bits.push('', 'HIGH-PRIORITY GAPS to pre-empt where this section can:');
    topGaps.slice(0, 6).forEach(g => {
      const title = g.title || g.gap || '';
      const action = g.suggested_action || g.fix || g.remedy || '';
      bits.push(`  · ${title}${action ? ` — mitigation: ${action}` : ''}`);
    });
  }
  ...
  if (narrative) {
    bits.push('', 'NARRATIVE POSITIONING ADVICE (apply where the section is narrative-led):');
    bits.push(`  ${narrative.slice(0, 800)}`);
  }

  // ── Team records — only those whose stated title / specialisms /
  // sectors plausibly fit the bid's service industry. Prevents an
  // IT/consulting bench being shoved into a film bid (or vice versa).
  // When no record fits the domain, the section prompt is told to fall
  // back to [TBC: <role>] markers using suggestedApproach phase roles
  // or generic role names appropriate to the service industry.
  const fittedTeam = filterTeamByDomainFit(Array.isArray(team) ? team : [], rfpData?.service_industry);
  const teamRecords = fittedTeam.slice(0, 20);
  if (teamRecords.length > 0) {
    bits.push('', `TEAM RECORDS (filtered for ${rfpData?.service_industry || 'this service domain'} — use real names in the Team section and anywhere a phase owner is referenced; only [TBC: <role>] when no member matches the role):`);
    teamRecords.forEach(m => {
      const specs = Array.isArray(m.stated_specialisms) ? m.stated_specialisms.slice(0, 4).join(', ') : (m.stated_specialisms || '');
      const sectors = m.stated_sectors || '';
      bits.push(`  · ${m.name} — ${m.title || 'role unknown'}${specs ? ` | specialisms: ${specs}` : ''}${sectors ? ` | sectors: ${sectors}` : ''}`);
    });
  } else if (Array.isArray(team) && team.length > 0) {
    // Members exist but none cleanly match the service domain. Previously
    // the prompt was told to use [TBC: <role>] for everything which
    // confused users — they had a team and were getting placeholders.
    // New rule: list everyone, ask the AI to name the best-fit person
    // with an explicit "[low domain fit — verify before submission]"
    // tag, falling back to [TBC] only when no member is even adjacent.
    bits.push('', `TEAM RECORDS — ${team.length} member(s) on file but none cleanly match the ${rfpData?.service_industry || 'service'} domain. Name the best-fit member from the list below with a "[low domain fit — verify before submission]" caveat, rather than substituting a generic [TBC: <role>] placeholder. Use [TBC] only when there's no plausible adjacency at all.`);
    team.slice(0, 20).forEach(m => {
      const specs = Array.isArray(m.stated_specialisms) ? m.stated_specialisms.slice(0, 4).join(', ') : (m.stated_specialisms || '');
      const sectors = m.stated_sectors || '';
      bits.push(`  · ${m.name} — ${m.title || 'role unknown'}${specs ? ` | specialisms: ${specs}` : ''}${sectors ? ` | sectors: ${sectors}` : ''}`);
    });
  } else {
    bits.push('', 'TEAM RECORDS: none supplied. The Team section MUST use [TBC: <role>] markers — these will be highlighted to the user for input.');
  }

  // ── Writing insights from matched proposals
  const insights = Array.isArray(writingInsights) ? writingInsights.slice(0, 4) : [];
  if (insights.length > 0) {
    bits.push('', 'WRITING INSIGHTS — patterns that worked in similar won bids (mirror the technique, not the content):');
    insights.forEach(w => {
  ...
  // ── Section contract: target depth + required content + forbidden
  if (contract && typeof contract === 'object') {
    bits.push('', `SECTION CONTRACT for "${sectionName}":`);
    if (contract.target_words) {
      bits.push(`  · Target depth: approximately ${contract.target_words} words. A section significantly shorter than this will be flagged for expansion in QA.`);
    }
    if (Array.isArray(contract.required_content) && contract.required_content.length > 0) {
      bits.push('  · Required content (each item MUST be present in the draft):');
      contract.required_content.forEach(r => bits.push(`      - ${r}`));
    }
    if (Array.isArray(contract.forbidden) && contract.forbidden.length > 0) {
      bits.push('  · Forbidden in this section:');
      contract.forbidden.forEach(f => bits.push(`      - ${f}`));
    }
  }

  if (bits.length === 0) return '';
  return `═══════════════════════════════════════════════════════════════════════════
REQUIRED INPUTS — apply these to this section
═══════════════════════════════════════════════════════════════════════════
The upstream pipeline produced these signals to be used by the bid drafter.
They are not optional context. Where the section can carry a signal, the
draft MUST apply it. Where it cannot, leave it for another section.
...
If any field fails the critique, rewrite. Then return final JSON after
  ...
  try {
    if (hasOpenAI()) {
      // Tier 1 — final senior-facing brief. One call per scan, output
      // ends up in board-prep packs. Premium reasoning earns its keep.
      const raw = await openaiGenerate(systemSuffix, userPrompt, 4500, 'generateExecutiveBidBrief');
      const parsed = safeJSON(raw);
...
Return ONLY valid JSON after </thinking>.`;
  ...
  // Flash-preferred (Wave 6 Phase 3): per-match verdict, ~$0.018/scan saving.
  const useFlashFirst = preferFlashFor('sanityCheckTopMatches');
  ...
  if (useFlashFirst) {
    ...
    } catch (e) { console.error('Gemini sanityCheckTopMatches failed, falling back to OpenAI:', e.message); }
  }

  try {
    if (hasOpenAI()) {
      const raw = await openaiGenerate(systemSuffix, userPrompt, 1500, 'sanityCheckTopMatches');
      const parsed = safeJSON(raw);
      if (parsed?.verdicts) return parsed.verdicts;
    }
  } catch (e) {
...
// ── Proposal section contracts ─────────────────────────────────────────
// Module-level so single-section endpoints can resolve a contract for a
// given section_id without duplicating the definition. Title is assembled
// at call time when it depends on rfpData (e.g. the why-us section).
const SECTION_CONTRACTS = {
  exec: {
    title: 'Executive Summary',
    description: 'One sentence framing the requirement. What you will deliver. Single strongest proof point. Why you specifically. Forward-looking statement.',
    target_words: 250,
    snippet_use_cases: ['executive summary', 'closing line'],
    required_content: [
      'Adapted opening_narrative from win strategy if supplied',
      'Single named strongest proof point (a real prior project, not a generic claim)',
      'Differentiator angle from win strategy threaded into one sentence',
    ],
    forbidden: ['Generic phrases (proven track record, world-class, etc.)', 'Repetition of the same client name across multiple sentences'],
  },
  understanding: {
    title: 'Understanding of the Requirement',
    description: 'Restate with insight. Separate explicit from implied. What is hard about this. What goes wrong when similar work fails.',
    target_words: 350,
    snippet_use_cases: ['understanding'],
    required_content: [
      'Explicit list of stated deliverables drawn from rfpData.deliverables',
      'At least two implied requirements not stated in the brief',
      'One paragraph on what makes this engagement difficult, beyond surface scope',
      'One short paragraph on what causes similar engagements to fail',
      'Explicit engagement with each item in rfpData.background_priorities — name the client repositioning narrative and the named investment areas, and show how the response factors them in',
    ],
    forbidden: ['Verbatim restatement of the RFP brief', 'Generic statements like "we understand"'],
  },
  approach: {
    title: 'Our Proposed Approach',
    description: 'Outcome framing. Methodology rationale. Full phased deliverables (every phase must be substantive, not a one-liner). Success measures. Pre-empt the top gap.',
    target_words: 1100,
    snippet_use_cases: ['approach', 'approach section', 'methodology'],
    required_content: [
      'Outcome statement (what the client will have at the end of engagement)',
      'Methodology rationale citing a named methodology or framework (named, not generic)',
      'EVERY phase from suggestedApproach.suggested_phases reproduced at substantive depth — phase name, duration, full activity list (not first 3 only), team_roles, rationale',
      'Success measures with explicit checkpoints (e.g. at 3 months, 6 months)',
      'Pre-empt the top high-priority gap from the gaps list with a concrete tactic',
      'Differentiator angle from win strategy made operational here',
      'Total proposed timeline MUST fit within rfpData.timeline_weeks; if internal phase sum exceeds, compress and state the compressed durations',
      'Engagement with rfpData.background_priorities — show where the response addresses each named client priority (sustainability, modernisation, etc.)',
    ],
    forbidden: ['One-line phase descriptions', 'Phases without team roles or rationale', 'Generic phrases', 'Proposing a timeline that exceeds the RFP-stated window'],
  },
  experience: {
    title: 'Relevant Experience & Case Studies',
    description: '2-3 specific engagements. For each: context, what delivered, measurable outcome, relevance to THIS requirement. Capability first, past client second as proof point.',
    target_words: 600,
    snippet_use_cases: ['credibility', 'experience'],
    required_content: [
      'Two to three distinct case studies (do NOT repeat the same client across all of them)',
      'For each case study: client name (real, not Example 1), project name, what was delivered, a measurable outcome (number, %, time), and explicit relevance to THIS bid',
      'Capability-first framing per case (capability stated, then past client cited as proof — not "we did X for Y so we can do it again")',
    ],
    forbidden: ['Repeating the same past client across multiple paragraphs', 'Case studies without a measurable outcome', 'Past clients as templates rather than proof points'],
  },
  team: {
    title: 'Our Team',
    description: 'Named individuals, full names, credentials, role on THIS engagement, track record on cited case studies. Use real team records when available; only mark TBC when no record matches the role.',
    target_words: 500,
    snippet_use_cases: ['team', 'credibility'],
    required_content: [
      'Real names from team records when a member with a matching title is supplied — never a [TBC] placeholder for a role we have someone for',
      'For each named member: title, specialisms, role on THIS engagement, one specific past project they worked on (drawn from team records or matched proposals)',
      'Members from suggestedApproach phase team_roles surfaced explicitly when available',
      '[TBC: <role>] only when no team record matches the required role — flagged so the user knows to fill it',
    ],
    forbidden: ['Inventing team members not in the supplied team records', 'Generic role descriptions ("brings deep expertise")'],
  },
  qa: {
    title: 'Quality Assurance & Risk Management',
    description: 'Named QA methodology. Specific risks for THIS engagement. Concrete mitigations. Escalation process. Delivery-confidence language: phase gates, approvals map, risk register, decision rights.',
    target_words: 500,
    snippet_use_cases: ['risk', 'governance', 'delivery confidence'],
    required_content: [
      'Named QA methodology (Agile QA, ISO 9001, named internal framework — not "we ensure quality")',
      'Every risk from win strategy risks_to_mitigate addressed with a concrete mitigation tactic',
      'At least two risks specific to this engagement beyond the strategy list',
      'Explicit governance / escalation model with named owners (or [TBC] roles)',
      'Concrete delivery-confidence language where the RFP evaluates it: phase gates with named approvals, risk register with contingency, decision rights, change-control mechanics. Use snippets tagged "delivery confidence" or "governance" when available; do not substitute platitudes.',
    ],
    forbidden: ['Generic QA platitudes', 'Risks without paired mitigations', 'Vague assurance ("we will ensure quality") where a specific governance/delivery mechanism is warranted'],
  },
  commercial: {
    title: 'Commercial Proposal',
    description: 'Pricing structure with explicit numbers from suggestedApproach budget, reconciled to the RFP currency, stated range, and stated timeline. Key assumptions. Inclusions/exclusions. How scope changes are handled.',
    target_words: 400,
    snippet_use_cases: ['commercial', 'value proposition'],
    required_content: [
      'Indicative budget LOW / MID / HIGH — actual numbers, not "tiered approach"',
      'Per-workstream breakdown when suggestedApproach.indicative_budget.breakdown is present — mirror each line (workstream, range, driver) rather than collapsing to a flat total',
      'Quote in the RFP\'s stated currency (rfpData.currency); if internal estimate is in another currency, convert and note the FX assumption',
      'Reconcile to the RFP\'s stated budget range (rfpData.budget_range): propose within the range, OR if proposing outside, explicitly justify the deviation in one sentence',
      'Any timeline figure quoted in the assumptions block MUST equal rfpData.timeline_weeks. Do NOT mention a longer timeline than the RFP states — the Approach section is constrained to that window and the Commercial section must agree.',
      'Day-rate basis if available from suggestedApproach (cite the role names and rates); where the deliverables fall outside rate card coverage (e.g. film production, shoot crew, post), state that openly and price from industry benchmarks',
      'Explicit assumptions (e.g. assumes English-language only, X locations, Y rounds of revision)',
      'Inclusions and exclusions list',
      'Scope-change handling',
    ],
    forbidden: ['Vague phrases like "tiered approach" without any number', 'Missing exclusions list', 'Quoting in a currency different from the RFP without acknowledgement', 'Quoting materially below the RFP\'s stated range without justification', 'Stating a timeline that exceeds rfpData.timeline_weeks (the Approach section already fits the RFP window; Commercial must match)', 'Collapsing a supplied workstream breakdown into a single flat total'],
  },
  why_us: {
    title: 'Why the Client Should Choose Us',
    description: 'Strongest differentiator. Clear best-fit reason. Confident close.',
    target_words: 250,
    snippet_use_cases: ['why us', 'closing line', 'value proposition'],
    required_content: [
      'Differentiator angle from win strategy stated directly',
      'A single named proof point that uniquely qualifies the org',
      'Explicit alignment with at least one rfpData.background_priorities item (e.g. the client\'s repositioning narrative or a named investment area)',
      'Forward-looking close — what the client gets in the first 30 days',
    ],
    forbidden: ['Generic platitudes', 'Repeating the same proof point as the executive summary verbatim'],
  },
};

// Look up a section contract by id (or by title via case-insensitive
// match). Returns the contract object plus the canonical title, or null.
function getSectionContract(sectionId) {
  if (!sectionId) return null;
  const key = String(sectionId).toLowerCase().trim();
  if (SECTION_CONTRACTS[key]) return SECTION_CONTRACTS[key];
  // Fallback: title match
  for (const [id, c] of Object.entries(SECTION_CONTRACTS)) {
    if (c.title.toLowerCase() === key) return c;
  }
  return null;
}
...
async function generateFullProposal({
  ...
  winningLanguage = [],
  narrativeAdvice = '',
  // writingInsights — caller-supplied rich AI-curated patterns from
  // narrative_advice.writing_insights. When provided we use these
  // verbatim (matching the single-section endpoint exactly). When
  // not provided we fall back to building a shallower version from
  // the matches' own evidence_density / win_indicators, which is the
  // legacy behaviour and produces visibly weaker drafts.
  writingInsights = null,
  ...
  // Build the per-RFP section list from the shared contracts. why_us
  // takes the client name in its title; everything else is static.
  const PROPOSAL_SECTIONS = [
    { id: 'exec', ...SECTION_CONTRACTS.exec },
    { id: 'understanding', ...SECTION_CONTRACTS.understanding },
    { id: 'approach', ...SECTION_CONTRACTS.approach },
    { id: 'experience', ...SECTION_CONTRACTS.experience },
    { id: 'team', ...SECTION_CONTRACTS.team },
    { id: 'qa', ...SECTION_CONTRACTS.qa },
    { id: 'commercial', ...SECTION_CONTRACTS.commercial },
    { id: 'why_us', ...SECTION_CONTRACTS.why_us, title: 'Why ' + (rfpData.client || 'the Client') + ' Should Choose Us' },
  ].map(s => ({
    id: s.id,
    title: s.title,
    ...
    contract: {
      ...
      snippet_use_cases: s.snippet_use_cases,
  ...
  // Per-section QA adjustments accumulate here so the caller can render
  // the same "N adjustments applied" footer it would for a single section.
  const allAdjustments = [];

  // Resolve writingInsights once — caller-supplied (curated by the
  // narrative-advice AI step) takes precedence; the per-match rebuild
  // is the fallback. Doing this outside the loop also avoids repeating
  // the same shallow build 8 times for no benefit.
  const resolvedWritingInsights = (Array.isArray(writingInsights) && writingInsights.length > 0)
    ? writingInsights
    : (matches || [])
  ...
  for (const section of PROPOSAL_SECTIONS) {
    try {
      ...
      const result = await generateSectionDraft(
        ...
        executiveBrief,
        orgProfile,
        { gaps, suggestedApproach, narrativeAdvice, writingInsights: resolvedWritingInsights, team, contract: section.contract }
      );

      let sectionText = result?.draft || '[EVIDENCE NEEDED: This section could not be generated. Please draft manually.]';
      let sectionAdjustments = [];

      // Per-section QA finalisation. Critical: previously QA only ran on
      // the concatenated full document with no contract, which compressed
      // long sections (Approach especially) to fit the rewrite token budget.
      // Running QA per section with that section's contract preserves each
      // section's target depth — a full proposal now produces output of the
      // same depth as 8 single-section drafts.
      if (result?.draft) {
        try {
          console.log(`[generateFullProposal] QA finalising: ${section.title}`);
          ...
          if (finalised?.text) {
            sectionText = finalised.text;
            sectionAdjustments = finalised.adjustments || [];
          }
        } catch (e) {
          console.error(`[generateFullProposal] QA failed on ${section.title} (non-fatal):`, e.message);
        }
      }

      completedSections.push({ title: section.title, text: sectionText, adjustments: sectionAdjustments });
      // Tag adjustments with the section so the UI footer reads cleanly.
      sectionAdjustments.forEach(a => allAdjustments.push({ ...a, section: section.title }));
    } catch (e) {
      console.error(`[generateFullProposal] Failed on ${section.title}:`, e.message);
      completedSections.push({ title: section.title, text: '[EVIDENCE NEEDED: Generation failed for this section.]', adjustments: [] });
    }
  }
  ...
  // Return the rich shape so the API can surface section-level adjustments;
  // legacy callers that only want the text can read .text.
  return {
    text: fullProposal,
    sections: completedSections,
    qa_adjustments: allAdjustments,
    qa_adjustments_count: allAdjustments.length,
  };
}
...
async function qaDetectIssues({ draftText, rfpText = '', rfpData = {}, matches = [], orgProfile = null, team = [], scope = 'section', contract = null }) {
...
Be thorough but not imaginative. Only flag issues backed by evidence you can cite from the supplied inputs. The full RFP text is provided — do not assume any content is missing from it.

Issue categories to check (use these exact type codes):

  deliverable_missing    A deliverable named in rfpData.deliverables is not explicitly addressed in the draft. Severity is ALWAYS "blocker" — these are hard contract gates from the RFP itself, never warnings. Name the deliverable back to the evaluator using the RFP's exact phrasing and show the response covers it. "Minimum 2 creative routes" MUST appear as two distinct labelled creative options (Creative Route 1 / Creative Route 2), not a single mention. If the creative work itself can't be invented, use placeholder structures with [EVIDENCE NEEDED] markers — but the structure must be present.
  ...
  client_as_template     A past client or project is positioned as the source/template for the new bid rather than as a proof point. Pattern: "our approach draws on our [Past Client] work, proving we can…" (bad). The fix is to lead with the capability and cite the past client second as evidence: "our approach draws on previous successes in [capability], notably with [Past Client]…".
  length_below_contract  A section's word count is significantly below its contract's target_words (less than ~60% of target). The section needs to be expanded — not by inventing content, but by surfacing more from the supplied inputs (matched proposal details, suggested approach phases, win strategy priorities, winning language adaptations).
  required_content_missing  A specific item from the section contract's required_content list is not present in the draft.
...
For each issue, give: type (from above), location (short quote from the draft or section name), description (one sentence — what's wrong), suggested_fix (one sentence — what would fix it, specifically referencing available inputs or requesting an [EVIDENCE NEEDED] marker).`;
  ...
  // Full RFP text — no cap. This is the authoritative source for
  // deliverables, structure, and directives; judgment must see all of it.
  const fullRfp = rfpText || '';
...
${(() => {
  ...
  const lines = ['═══ SECTION CONTRACT ═══'];
  if (contract.target_words) lines.push(`Target depth: ~${contract.target_words} words. Flag length_below_contract if the draft is under ~${Math.round(contract.target_words * 0.6)} words.`);
  if (Array.isArray(contract.required_content) && contract.required_content.length > 0) {
    lines.push('Required content (flag required_content_missing for any item not present):');
    contract.required_content.forEach(r => lines.push(`  · ${r}`));
  }
  if (Array.isArray(contract.forbidden) && contract.forbidden.length > 0) {
    lines.push('Forbidden in this section:');
    contract.forbidden.forEach(f => lines.push(`  · ${f}`));
  }
  return lines.join('\n');
})()}
...
${buildOrgProfileBlock(orgProfile)}`;
  ...
  // Flash-preferred (Wave 6 Phase 3): catalogue-style issue detection
  // (16 issue types). Detection is pattern-matching, not writing —
  // Flash is fine here. The REWRITE pass (qaRewriteDraft below) stays
  // on OpenAI because it's the user-visible prose. Saves ~$0.056 per
  // section × 8 sections per full proposal = ~$0.45/proposal.
  const useFlashFirst = preferFlashFor('qaDetectIssues');
  const oaiUserPrompt = `${context}\n\nList every issue. Be thorough. Cite the draft or input in each description. Return ONLY valid JSON:\n${schema}`;
  const geminiPrompt = `${systemSuffix}\n\n${context}\n\nList every issue. Return ONLY valid JSON:\n${schema}`;
  ...
  if (useFlashFirst) {
    try {
      ...
      const parsed = safeJSON(raw);
      if (parsed) return parsed;
    } catch (e) { console.error('Gemini qaDetectIssues failed, falling back to OpenAI:', e.message); }
  }

  try {
    if (hasOpenAI()) {
      // Tier 5 — rule-application QA. o4-mini's reasoning catches edge
      // cases gpt-5.5 misses on strict checklists, at a fraction of the
      // pro-tier cost.
      const raw = await openaiGenerate(systemSuffix, oaiUserPrompt, 4500, 'qaDetectIssues');
      const parsed = safeJSON(raw);
      if (parsed) return parsed;
    }
...
// short change log.
async function qaRewriteDraft({ draftText, issues = [], rfpText = '', rfpData = {}, matches = [], orgProfile = null, team = [], scope = 'section', contract = null }) {
  if (!draftText || !draftText.trim()) return null;
...
11. For client_as_template hits, restructure the sentence so the
    capability or approach is stated FIRST in the org's own voice, and
    the past client is cited SECOND as evidence the capability has
    already been delivered. Do not delete the client reference — it is
    valuable proof; only its grammatical role changes (from source of
    the approach → proof point validating the approach). Preserve the
    underlying claim.

12. For length_below_contract hits, EXPAND the section toward its
    target word count using only material from the supplied inputs.
    Sources to draw from in priority order:
      (a) Suggested approach phases — name, duration, full activity
          list, team_roles, rationale, deliverables. Each phase
          deserves substantive paragraph treatment, not a one-liner.
      (b) Win strategy — opening_narrative, top_priorities (each with
          why_it_matters and evidence), risks_to_mitigate (with
          mitigations), differentiator_angle, emphasise list.
      (c) Matched proposals — themes, methodologies, deliverables,
          standout sentences (as patterns, not lifted facts).
      (d) Winning language snippets — adapt using how_to_adapt
          guidance.
      (e) Team records — real names matched to phase roles.
    Do not invent content to hit the target. If after exhausting the
    supplied inputs the section is still under target, leave it and
    note in the adjustments log that more source material is needed.

13. For required_content_missing hits, add the missing item to the
    section using available inputs. Do NOT acknowledge the gap in the
    prose — just integrate the content. If the input doesn't support
    the requirement, insert an [EVIDENCE NEEDED] marker for the
    specific data needed.

14. For deliverable_missing hits, add an explicit response to the
    deliverable in the section that logically owns it. The deliverable
    must be named back to the evaluator (using the RFP's exact
    phrasing where useful) and the response must cover it concretely.
    Where the deliverable specifies a quantity ("minimum 2 routes"),
    deliver that quantity — placeholder structures count if the
    creative work itself can't be invented (e.g. "Creative Route 1:
    [EVIDENCE NEEDED: route concept and visual direction]" + the
    same for Route 2). Do not collapse multiple routes into one.
...
Return the corrected text in full, plus a short list of the adjustments you actually made (one line per adjustment, categorised by issue type).`;
  ...
  // Full RFP text — no cap. See qaDetectIssues for rationale.
  const fullRfp = rfpText || '';
...
${(() => {
  if (!contract || typeof contract !== 'object') return '';
  const lines = ['SECTION CONTRACT (use to drive expansion targets):'];
  if (contract.target_words) lines.push(`  target_words: ~${contract.target_words}`);
  if (Array.isArray(contract.required_content)) {
    lines.push('  required_content:');
    contract.required_content.forEach(r => lines.push(`    · ${r}`));
  }
...
// ── AMEND SECTION DRAFT (natural-language revise) ──────────────────────
// Takes a current section draft + a free-text user instruction (e.g.
// "make this more confident", "add a paragraph about our Arabic-speaking
// team", "remove the second case study") and returns a revised draft.
//
// Critical constraints baked into the system prompt:
//   · Apply ONLY the requested change. Do not rewrite untouched text.
//   · Preserve [#N] match citations and [Lk] winning-language citations
//     exactly where they appear unless the instruction explicitly asks
//     to add, remove, or move one.
//   · Preserve the voice/register of the existing draft.
//   · If the instruction cannot be safely applied without inventing
//     facts not in the source context, return the original text and
//     explain in `change_summary`.
//
// Returns { text, change_summary } — text is the revised draft, ready to
// drop into section_drafts.draft_text. change_summary is a one-line
// description of what changed, for UI feedback / audit.
async function amendSectionDraft(currentText, instruction, rfpData, matchedProposals, winningLanguage, orgProfile = null, opts = {}) {
  if (!currentText || !instruction) {
    return { text: currentText || '', change_summary: 'no-op' };
  }
  const matches = (matchedProposals || []).slice(0, 6);
  const matchContext = matches
    .map((m, i) => `[#${i + 1}] ${m.name} · ${m.client || 'Unknown client'} · ${m.outcome || 'pending'}${m.user_rating ? ` · rating ${m.user_rating}/5` : ''}`)
    .join('\n');
  const langContext = (winningLanguage || []).slice(0, 8)
    .map((l, i) => `[L${i + 1}] ${l.snippet || l.text || ''}`)
    .filter(s => s.length > 6)
    .join('\n');
  const orgBlock = orgProfile?.confirmed_profile
    ? `\nORG CONTEXT:\n${JSON.stringify(orgProfile.confirmed_profile).slice(0, 1200)}\n`
    : '';

  const systemSuffix = `You are revising a single section of a proposal draft based on the user's natural-language instruction.

CRITICAL RULES:
1. Apply ONLY the requested change. Do NOT rewrite untouched text.
2. Preserve [#N] match citations and [Lk] winning-language citations exactly where they appear, unless the instruction explicitly asks to add, remove, or move one.
3. Preserve the voice/register of the existing draft.
4. Never invent facts not present in the source context (matches, winning language, org profile). If the instruction asks for content you cannot ground, leave the draft unchanged and explain in change_summary.
5. Keep [EVIDENCE NEEDED] markers in place unless the instruction explicitly replaces them.
...
═══════════════════════════════════════════════════════════════
CURRENT DRAFT (this is what to revise)
═══════════════════════════════════════════════════════════════
${currentText}

═══════════════════════════════════════════════════════════════
SOURCE CONTEXT — facts you may cite. Anything outside this set is invention.
═══════════════════════════════════════════════════════════════
RFP brief: ${(rfpData?.brief_summary || rfpData?.summary || '').slice(0, 600)}
RFP requirements: ${JSON.stringify((rfpData?.requirements || []).slice(0, 8)).slice(0, 1000)}

Matched past proposals:
${matchContext || '(none)'}

Winning-language snippets:
${langContext || '(none)'}
${orgBlock}
═══════════════════════════════════════════════════════════════
TASK
═══════════════════════════════════════════════════════════════
Revise the draft above per the user instruction. Return:
{
  "text": "the full revised draft (string). If the instruction can't be safely applied, return the original text unchanged.",
  "change_summary": "one short sentence describing what changed (or 'no change applied — reason' if nothing changed)"
}`;

  try {
    if (hasOpenAI()) {
      const raw = await openaiGenerate(systemSuffix, userPrompt, 4000, 'amendSectionDraft');
      const parsed = safeJSON(raw);
      if (parsed?.text) {
        return {
          text: parsed.text,
          change_summary: parsed.change_summary || 'revised',
        };
      }
    }
  } catch (e) {
    console.error('OpenAI amendSectionDraft failed, falling back to Gemini:', e.message);
  }
  ...
  try {
    ...
    if (parsed?.text) {
      return {
        text: parsed.text,
        change_summary: parsed.change_summary || 'revised (gemini fallback)',
      };
    }
  } catch (e) {
    console.error('amendSectionDraft Gemini fallback failed:', e.message);
  }

  // Safe degrade — return original text unchanged, surface as no-op
  return {
    text: currentText,
    change_summary: 'no change applied — generation failed, please retry',
  };
}
...
${schema}`;
  ...
  try {
    const raw = await openaiGenerate(systemSuffix, userPrompt, 3000, 'checkRequirementsCoverage');
    return safeJSON(raw);
  } catch (e) {
