...
EVALUATION: ${(rfpData.evaluation_logic || []).join('; ') || 'not extracted'}
MUST requirements: ${(rfpData.requirements || []).filter(r => r.priority === 'must').map(r => r.text).join(' | ') || 'none'}
SHOULD requirements: ${(rfpData.requirements || []).filter(r => r.priority === 'should').slice(0, 8).map(r => r.text).join(' | ') || 'none'}`;
...
3. ## Our Proposed Approach (300-500 words)
   Methodology, phasing, delivery logic. Ground it in what worked in
   similar projects [#N]. Include phases with rationale. Address the
   high-priority gaps proactively.
...
═══════════════════════════════════════════════════════════════════════════
TASK: Write the complete proposal now. All 8 sections, in order, with
## markdown headers. Write it as if the client will read it tomorrow.
═══════════════════════════════════════════════════════════════════════════`;

  try {
    if (hasOpenAI()) {
      const raw = await openaiGenerate(systemSuffix, userPrompt, 8000);
