...
Return the final JSON after </thinking>. Return ONLY valid JSON in your final answer (no prose, no code fences).`;
  ...
  try {
    if (hasOpenAI()) {
      ...
      const raw = await openaiGenerate(systemSuffix, userPrompt, 6000, 'analyseProposal', MODEL_DEEP);
...
If any field fails the critique, rewrite. Then return final JSON after
  ...
  try {
    if (hasOpenAI()) {
      ...
      const raw = await openaiGenerate(systemSuffix, userPrompt, 4500, 'generateExecutiveBidBrief', MODEL_DEEP);
      const parsed = safeJSON(raw);
...
Return ONLY valid JSON after </thinking>.`;
  ...
  try {
    if (hasOpenAI()) {
      const raw = await openaiGenerate(systemSuffix, userPrompt, 1500, 'sanityCheckTopMatches', MODEL_REASONING);
      const parsed = safeJSON(raw);
      if (parsed?.verdicts) return parsed.verdicts;
    }
  } catch (e) {
...
${buildOrgProfileBlock(orgProfile)}`;
  ...
  try {
    if (hasOpenAI()) {
      ...
      // pro-tier cost.
      const raw = await openaiGenerate(systemSuffix, oaiUserPrompt, 4500, 'qaDetectIssues', MODEL_REASONING);
      const parsed = safeJSON(raw);
      if (parsed) return parsed;
    }
...
${schema}`;
  ...
  try {
    const raw = await openaiGenerate(systemSuffix, userPrompt, 3000, 'checkRequirementsCoverage', MODEL_REASONING);
    return safeJSON(raw);
  } catch (e) {
