...
${(() => {
  ...
  if (teamRecords.length > 0) {
    ...
    bits.push('', `TEAM RECORDS: ${team.length} member(s) on file but none match the ${rfpData?.service_industry || 'service'} domain. DO NOT name members from outside the domain — use [TBC: <role>] markers for every named role, drawn from the suggestedApproach phase team_roles or canonical roles for this service domain (e.g. for film: Director, Executive Producer, DoP, Editor, Producer; for design: Creative Director, Designer; etc.). Markers are coloured for the user to fill in later.`);
  } else {
