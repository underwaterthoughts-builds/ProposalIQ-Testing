...
- Zero named methodologies (PRINCE2, Agile/SAFe, FHIR, DAMA-DMBOK, etc.):
  approach_quality.methodology_clarity ≤ 50
- Zero risks named or addressed anywhere in the document:
  approach_quality.risk_acknowledgement ≤ 25
...
score is too high.`;
  ...
  "writing_quality": {
    "overall_score": "integer 0-100 per the calibration bands. Apply hard caps before scoring.",
  ...
  "credibility_signals": {
    "overall_score": "integer 0-100. Max 35 if experience section uses Example 1/2/3 placeholders or no real client names.",
...
async function analyseWinPatterns(wonProjects, lostProjects, opts = {}) {
  ...
  const { orgProfile = null, sectorStats = [], clientStats = [], scope = 'repository' } = opts;
  ...
  const summarise = (projects) => pickSample(projects).map(p => {
    ...
    return `- "${p.name}" — ${p.client || 'unknown client'} (${taxonomy}, ${value}, ${year}, rating=${p.user_rating || 0}/5): writing=${wq.overall_score || '?'}/100, approach=${aq.overall_score || '?'}/100, credibility=${cq.overall_score || '?'}/100 | themes: ${(meta.key_themes || []).slice(0, 4).join(', ') || 'none'} | methodologies: ${(meta.methodologies || []).slice(0, 3).join(', ') || 'none'} | win/loss indicators: ${(meta.win_indicators || meta.loss_indicators || []).slice(0, 2).join('; ') || 'none'}`;
  }).join('\n');
  ...
  const systemSuffix = `You are a bid analytics consultant producing an ORGANISATION-LEVEL portfolio analysis across a set of won and lost proposals.

This is a company-wide view, not project-by-project. Your job is to surface the patterns that should change how this organisation positions, scopes, and writes future bids.

Rules:
- Be specific. Reference actual score differences, sector win rates, recurring themes, named clients where useful.
- Cite evidence from the data you were given — do not invent numbers.
- Do not state the obvious ("winners write better than losers"). Only surface patterns that would change behaviour.
- Call out concentration risk (one client dominating wins, one sector dominating losses) when the numbers show it.
- Ground every recommendation in the organisation's actual offerings and differentiators when present; flag when bids are drifting outside the stated profile.
- Where the data is too thin for a confident claim, say "insufficient data" rather than speculate.`;

  const schema = `{
  "headline": "one-sentence org-level verdict that captures the most important pattern",
  "strengths": [
    { "title": "short label", "evidence": "specific cited evidence from the sample", "leverage": "how to lean into this in future bids" }
  ],
  "weaknesses": [
    { "title": "short label", "evidence": "specific cited evidence", "remedy": "concrete change to address it" }
  ],
  "concentration_risks": [
    { "risk": "e.g. 60% of wins come from one client", "severity": "low|medium|high", "action": "what to do about it" }
  ],
  "sector_strengths": [{ "sector": "name", "win_rate": 72, "observation": "why this sector works" }],
  "sector_weaknesses": [{ "sector": "name", "win_rate": 18, "observation": "why bids fail here" }],
  "recurring_losing_patterns": [
    { "pattern": "short label", "evidence": "specific cited evidence", "fix": "concrete change" }
  ],
  ...
  "writing_insight": "one specific, actionable insight about writing quality differences between won and lost",
