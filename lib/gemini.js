...
function filterTeamByDomainFit(team, serviceIndustry) {
  ...
  if (!serviceIndustry) return team;
  ...
  const KEYWORDS = {
    ...
    'advertising & campaigns': ['creative director', 'art director', 'copywriter', 'strategist', 'planner', 'producer', 'campaign'],
  ...
  if (wanted.length === 0) return team; // unknown domain — let the prompt decide
...
const SECTION_CONTRACTS = {
  ...
  commercial: {
    ...
    description: 'Pricing structure with explicit numbers from suggestedApproach budget, reconciled to the RFP currency and stated range. Key assumptions. Inclusions/exclusions. How scope changes are handled.',
    target_words: 400,
    ...
    forbidden: ['Vague phrases like "tiered approach" without any number', 'Missing exclusions list', 'Quoting in a currency different from the RFP without acknowledgement', 'Quoting materially below the RFP\'s stated range without justification'],
  },
  why_us: {
...
Issue categories to check (use these exact type codes):
  ...
  deliverable_missing    A deliverable named in rfpData.deliverables is not explicitly addressed in the draft. Each deliverable is a hard contract gate: name it back to the evaluator and show the response covers it. "Minimum 2 creative routes", for example, MUST appear as two distinct creative options, not a single mention.
