  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (usage) {
        ...
        if (cached > 0) {
          console.log(`[ai-cost ${_fnName}] cached_tokens=${cached} of ${usage.prompt_tokens} input (~${Math.round(100 * cached / usage.prompt_tokens)}% cache hit)`);
        }
...
function filterTeamByDomainFit(team, serviceIndustry) {
  ...
  console.log(`[filterTeamByDomainFit] service_industry="${serviceIndustry}" — ${result.length}/${team.length} members passed`);
  return result;
}
...
async function analyseProposal(text, rating, userNotes, ctx = {}) {
  ...
  console.log(`[analyseProposal] detected work-type="${workType}" (sector="${ctx.sector || ''}", service_industry="${ctx.service_industry || ''}")`);
...
async function getIndustryNews(rfpData, rfpText) {
  ...
  console.log(`[market-context] mode=${isProcurement ? 'procurement' : 'creative'} client="${rfpData?.client || '?'}" sector="${rfpData?.sector || '?'}"`);
