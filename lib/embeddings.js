...
// Per-rating weight used by rankTeamMembers when scoring participation
// history. Mirrors the legacy lib/rating.js scale; previously this was
// referenced without being defined, so any project_team row threw a
// ReferenceError that was silently caught upstream — team-history
// boost was permanently zero. Defining locally keeps the dependency
// contained.
...
// ── Similarity calibration for the embedding model in use ──────────────────
// gemini-embedding-2 cosines cluster in ~0.55–0.85 for this corpus, where the
// retired OpenAI text-embedding-3-small spread ~0.15–0.55 — the band every
// downstream constant (matchLabel thresholds, the ×130 score mapping, team
// fit) was tuned against. Without recalibration every candidate saturates at
// the 99 cap and rates Strong (observed on the first Gemini-primary scan:
// 12 matches at 99, a cross-domain KM proposal at 99 from a 0.69 cosine).
// Anchors: 0.80→0.77 (Strong), 0.69→0.50 (Good), 0.60→0.29 (Partial).
// Re-derive these anchors if EMB_MODELS changes again.
function calibrateSimilarity(raw) {
  return Math.max(0, Math.min(1, (raw - 0.48) * 2.4));
}

// Match label based on (calibrated) similarity score
...
function sectorBonus(proposal, rfp) {
  ...
  if (proposal.client_industry && rfp.client_industry &&
    ...
    const propSecs = parseJsonField(proposal.client_sectors, []) || [];
  ...
  if (proposal.service_industry && rfp.service_industry &&
    ...
    const propSecs = parseJsonField(proposal.service_sectors, []) || [];
...
function rankProposals(queryVec, projects, rfpTaxonomy, ownerUserId = null) {
  ...
  // ownerUserId: tenant-scopes the feedback signal so member A's ranking
  // never gets boosted by tenant B's usage history. null = unrestricted
  // (admin contexts).
  let usageStats = new Map();
  try {
    const { getProjectUsageStats } = require('./feedback');
    usageStats = getProjectUsageStats(null, ownerUserId);
  } catch (e) {
  ...
  return projects
    ...
    .map(p => {
      const vec = parseJsonField(p.embedding, null);
      if (!vec) return null;
      const raw = calibrateSimilarity(cosine(queryVec, vec));
      ...
      const aiMeta = parseJsonField(p.ai_metadata, {});
      return {
        ...p,
        ai_metadata: aiMeta,
        taxonomy: parseJsonField(p.taxonomy, {}),
        raw_similarity: Math.round(raw * 100),
        // Quality boosts (rating, outcome, KQS) can push a mid-similarity
        // proposal's weighted score far above its actual relevance — cap the
        // displayed score inside its label band so a "Good" match can never
        // show 97 (observed: cross-domain KM proposal, calibrated sim 0.52,
        // boosted to 97). Bands: Strong ≤99, Good ≤84, Partial ≤64, Weak ≤39.
        match_score: Math.min(
          Math.round(weighted * 130),
          { Strong: 99, Good: 84, Partial: 64, Weak: 39 }[matchLabel(raw)] ?? 99
        ),
...
function rankTeamMembers(queryVec, members, participationHistory) {
  return members
    ...
    .map(m => {
      const vec = parseJsonField(m.embedding, null);
      if (!vec) return null;
      const cvSim = calibrateSimilarity(cosine(queryVec, vec));
      ...
      const cvExtracted = parseJsonField(m.cv_extracted, {});
...
module.exports = { cosine, rankProposals, rankTeamMembers, parseJsonField, matchLabel };
