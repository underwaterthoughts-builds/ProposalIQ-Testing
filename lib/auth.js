...
// JWT secret hard-fail (Wave 6 Phase 3 hardening). Previously fell back
// to 'proposaliq-dev-secret-change-me', which is in source — anyone
// could mint admin tokens against a misconfigured deployment. Now we
// throw at boot so the deployment fails loudly instead of silently
// exposing every account. NODE_ENV !== 'production' allows a dev
// fallback so local dev without env vars still works.
const SECRET = (() => {
  const fromEnv = process.env.JWT_SECRET;
  if (fromEnv && fromEnv.length >= 32) return fromEnv;
  if (process.env.NODE_ENV !== 'production') {
    return 'proposaliq-dev-secret-change-me-this-is-32+chars';
  }
  throw new Error(
    'JWT_SECRET environment variable is required in production and must be at least 32 characters. ' +
    'Set it in Railway → Variables before starting the service.'
  );
})();
