...
//
// ownerUserId scopes results to events from a single tenant's scans.
// Pass null (or omit) for the legacy global aggregate — only safe in
// admin contexts. Multi-tenant callers MUST pass the requesting user's
// id; otherwise one tenant's feedback signal can bias another tenant's
// rankings (Wave 6 Phase 3 — feedback was the last cross-tenant leak
// in the ranking path).
function getProjectUsageStats(db = null, ownerUserId = null) {
  const _db = db || getDb();
  try {
    const tenantClause = ownerUserId
      ? ' AND s.owner_user_id = ?'
      : '';
    ...
    const rows = _db.prepare(`
      ...
      LEFT JOIN rfp_scans s ON s.id = u.scan_id
      WHERE u.target_type = 'project'
        AND u.target_id IS NOT NULL${tenantClause}
...
// ownerUserId scopes to events from one tenant; see getProjectUsageStats.
function getSnippetUsageStats(db = null, ownerUserId = null) {
  const _db = db || getDb();
  try {
    const tenantClause = ownerUserId ? ' AND s.owner_user_id = ?' : '';
    const params = ownerUserId ? [ownerUserId] : [];
    const rows = _db.prepare(`
      SELECT
