...
const { parseJsonField, cosine } = require('./embeddings');
...
async function getProject(input, { user }) {
  ...
  return { ...row, ai_metadata: parseJsonField(row.ai_metadata, {}) };
}
...
async function searchProjects(input, { user }) {
  ...
  const scored = candidates
    .map(p => {
      ...
      return { id: p.id, name: p.name, client: p.client, sector: p.sector, outcome: p.outcome, ai_metadata: parseJsonField(p.ai_metadata, {}), similarity: Math.round(sim * 100) };
    })
    .filter(Boolean)
...
async function listScans(input, { user }) {
  ...
  return {
    count: rows.length,
    scans: rows.map(s => {
      const bs = s.bid_score ? parseJsonField(s.bid_score, null) : null;
      return {
        id: s.id,
        name: s.name,
        status: s.status,
...
async function getScan(input, { user }) {
  ...
  return {
    ...
    name: row.name,
    status: row.status,
    rfp_data: parseJsonField(row.rfp_data, {}),
    bid_score: parseJsonField(row.bid_score, null),
    matched_proposals: (parseJsonField(row.matched_proposals, []) || []).slice(0, 8),
    gaps: parseJsonField(row.gaps, []),
    win_strategy: parseJsonField(row.win_strategy, null),
    winning_language: (parseJsonField(row.winning_language, []) || []).slice(0, 6),
    suggested_approach: parseJsonField(row.suggested_approach, null),
    executive_brief: parseJsonField(row.executive_brief, null),
    team_suggestions: (parseJsonField(row.team_suggestions, []) || []).slice(0, 6),
  };
}
...
async function listTeam(input, { user }) {
  ...
  return {
    count: rows.length,
    members: rows.map(m => ({ ...m, stated_specialisms: parseJsonField(m.stated_specialisms, []) })),
  };
}
