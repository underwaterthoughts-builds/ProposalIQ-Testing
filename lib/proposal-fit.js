...
const { AI_ANALYSIS_TIMEOUT_MS, PARSE_TIMEOUT_MS, withTimeout } = require('./timeouts');
...
function computeCoverageScore(rows) {
  ...
  for (const r of rows) {
    ...
    possible += w * 100;
    if (r.user_override === 'covered') {
      // User asserted "we have this". Strong credit, but not full marks —
      // the capability exists yet isn't evidenced in the response text,
      // and the score should still nudge them to write it in.
      earned += w * Math.max(85, r.strength_score || 0);
    }
    else if (r.status === 'addressed') earned += w * (r.strength_score || 80);
...
// Recompute the composite fit after a coverage override changes, reusing
// the non-coverage dimension scores persisted at analysis time. Returns
// the new overall + coverage score.
function recomputeFitOverall(db, scanId) {
  const scan = db.prepare('SELECT proposal_metadata FROM rfp_scans WHERE id = ?').get(scanId);
  let metadata = {};
  try { metadata = JSON.parse(scan?.proposal_metadata || '{}'); } catch {}
  const s = metadata._scores || {};
  const rows = db.prepare('SELECT * FROM proposal_coverage WHERE scan_id = ?').all(scanId);
  const coverage = rows.length ? computeCoverageScore(rows) : (s.coverage ?? null);
  ...
  metadata._scores = { ...s, coverage, overall };
  db.prepare('UPDATE rfp_scans SET proposal_metadata = ?, proposal_fit_overall = ? WHERE id = ?')
    .run(JSON.stringify(metadata), overall, scanId);
...
async function analyseProposalAgainstRfp(scanId) {
  ...
  // Multi-document support: a response is often split across commercial /
  // technical / annex files. All docs for the scan are parsed and combined
  // (with per-document headers so the AI can attribute content). Legacy
  // single-doc scans fall back to the proposal_filename column.
  let docs = [];
  try {
    docs = db.prepare('SELECT filename, original_name FROM rfp_scan_proposal_docs WHERE scan_id = ? ORDER BY created_at').all(scanId);
  } catch {}
  if (docs.length === 0 && scan.proposal_filename) {
    docs = [{ filename: scan.proposal_filename, original_name: scan.proposal_original_name || scan.proposal_filename }];
  }
  if (docs.length === 0) throw new Error('no proposal attached');
  ...
  // Combined budget: keep per-requirement prompts affordable. Each doc gets
  // an even share of the overall cap (never below a useful floor).
  const COMBINED_CAP = 60000;
  const perDocCap = Math.max(10000, Math.floor(COMBINED_CAP / docs.length));

  const parsedParts = [];
  const parseErrors = [];
  for (const doc of docs) {
    const p = path.join(process.cwd(), 'data', 'uploads', 'rfp_scans', doc.filename);
    if (!fs.existsSync(p)) { parseErrors.push(`${doc.original_name}: file not on disk`); continue; }
    try {
      const text = await withTimeout(parseDocument(p), PARSE_TIMEOUT_MS, `parseProposal:${doc.original_name}`);
      if (text && text.trim().length >= 50) {
        parsedParts.push(
          `═══ DOCUMENT ${parsedParts.length + 1} of ${docs.length}: ${doc.original_name} ═══\n` +
          text.slice(0, perDocCap)
        );
      } else {
        parseErrors.push(`${doc.original_name}: no readable text`);
      }
    } catch (e) {
      parseErrors.push(`${doc.original_name}: ${e.message}`);
    }
  }

  if (parsedParts.length === 0) {
    const detail = parseErrors.join('; ').slice(0, 200) || 'no readable documents';
    setStatus(db, scanId, 'error', `parse failed: ${detail}`);
    throw new Error('proposal parse failed: ' + detail);
  }
  if (parseErrors.length > 0) {
    console.warn(`[proposal-fit ${scanId}] some documents unreadable:`, parseErrors.join('; '));
  }

  const proposalText = parsedParts.join('\n\n').slice(0, COMBINED_CAP);
  ...
  // "We have this" assertions survive a re-run: the AI re-grades the text,
  // but a user-asserted capability stays asserted (requirements come from
  // the same rfp_data, so requirement_index is stable).
  let priorOverrides = [];
  try {
    priorOverrides = db.prepare(
      'SELECT requirement_index, user_override, override_note FROM proposal_coverage WHERE scan_id = ? AND user_override IS NOT NULL'
    ).all(scanId);
  } catch {}
  if (hasRequirements) {
    ...
    for (const o of priorOverrides) {
      try {
        db.prepare('UPDATE proposal_coverage SET user_override = ?, override_note = ? WHERE scan_id = ? AND requirement_index = ?')
          .run(o.user_override, o.override_note, scanId, o.requirement_index);
      } catch {}
    }
  }
