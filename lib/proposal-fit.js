...
// Composite fit score — weighted blend of the dimensions the spec called
// out. All inputs 0-100; output 0-100 integer.
function compositeFit({ coverage, methodology, evidence, pricing, criteria }) {
  ...
  const score =
    ...
    (methodology || 0) * w.methodology +
    (evidence    || 0) * w.evidence +
    (pricing     || 0) * w.pricing +
    (criteria    || 0) * w.criteria;
...
async function analyseProposalAgainstRfp(scanId) {
  ...
  if (criteria.length > 0) {
    ...
    if (scores.length) criteriaScore = Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
  }

  // Step 4: composite
  const coverageRows = db.prepare('SELECT * FROM proposal_coverage WHERE scan_id = ?').all(scanId);
  const coverageScore = computeCoverageScore(coverageRows);
  const methodologyScore = metadata?.approach_quality?.methodology_clarity || 0;
  const evidenceScore = Math.round(((metadata?.writing_quality?.specificity_score || 0)
                                  + (metadata?.writing_quality?.evidence_density || 0)) / 2);
  ...
  const overall = compositeFit({
    ...
    pricing:     pricingScore == null ? methodologyScore : pricingScore,
    criteria:    criteriaScore || coverageScore,
  });
  ...
  return { overall, coverageScore, methodologyScore, evidenceScore, pricingScore, criteriaScore };
}
