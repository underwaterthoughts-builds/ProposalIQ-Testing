...
// Detect when analyseProposal returned its empty fallbackMeta shape rather
// than a real analysis. Both writing/approach/credibility scores zero AND
// no key_themes / methodologies signals → the AI calls failed internally
// (logged to Railway as console.error) and we got back the safe-but-empty
// shell. Surface this so the UI can show "Analysis failed — re-run"
// instead of silently hiding all the score panels.
function isFallbackMain(main) {
  if (!main || typeof main !== 'object') return true;
  const wq = main.writing_quality || {};
  const aq = main.approach_quality || {};
  const cs = main.credibility_signals || {};
  const allZero = (wq.overall_score || 0) === 0
               && (aq.overall_score || 0) === 0
               && (cs.overall_score || 0) === 0;
  const noContent = (!Array.isArray(main.key_themes) || main.key_themes.length === 0)
                 && (!Array.isArray(main.methodologies) || main.methodologies.length === 0)
                 && (!Array.isArray(main.deliverables) || main.deliverables.length === 0);
  return allZero && noContent;
}

function synthesiseProject(perDocAnalyses) {
  // perDocAnalyses: array of { filename, subtype, analysis }
  const mainEntry = perDocAnalyses.find(d => d.subtype === 'main_proposal' && d.analysis && !d.analysis._error);
  const main = mainEntry?.analysis || {};
  const result = { ...main };
  // Flag for the UI when main_proposal didn't really analyse — distinct
  // from "no main_proposal in the upload at all".
  if (mainEntry && isFallbackMain(main)) {
    result._main_analysis_failed = true;
    result._main_analysis_filename = mainEntry.filename;
  } else if (!mainEntry && perDocAnalyses.some(d => d.subtype === 'main_proposal')) {
    result._main_analysis_failed = true;
    const failed = perDocAnalyses.find(d => d.subtype === 'main_proposal');
    result._main_analysis_filename = failed?.filename || null;
  }
