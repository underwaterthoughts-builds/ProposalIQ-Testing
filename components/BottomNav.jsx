...
export default function BottomNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 border-t bottom-nav-safe md:hidden"
      style={{ background: '#141311', borderColor: '#4d4636' }}
    >
      {/* Quick/Pro toggle strip — only on relevant pages */}
      {showModeSwitcher && (
        <div className="flex border-b" style={{ borderColor: '#4d4636' }}>
      ...
      <div className="flex">
        {NAV.map(item => {
          ...
          const colour = active ? '#e8c357' : '#9b8e80';
          return (
            <Link
              key={item.href}
              href={item.href}
              ...
              style={{ color: colour, minHeight: '56px' }}
            >
              <span
              ...
              <span className="text-[9px] font-medium tracking-wide" style={{ color: colour }}>
                {item.label}
              </span>
              {active && (
                <div className="absolute bottom-0 w-6 h-0.5 rounded-full" style={{ background: '#e8c357' }} />
              )}
            </Link>
          );
        })}
